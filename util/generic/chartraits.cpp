...
static inline const char* FindChr(const char* s, char c, size_t len) noexcept {
    const char* ret = TCharTraits<char>::Find(s, c, len);

    return ret ? ret : (s + len);
}

static inline const char* FindTwo(const char* s, const char* c, size_t len) noexcept {
    const char* e = s + len;
