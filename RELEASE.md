...
## New features:
- With this release we support `Text` features for *classification on GPU*. To make a text feature, simply use `Text` as the column type in column description file. Achieve better quality by using text information of your dataset. See more in [Learning CatBoost with text features](https://github.com/catboost/tutorials/blob/master/text_features/text_features_in_catboost.ipynb)
