...
static std::set<TString> java_keywords = {
  "abstract",
  "assert",
  "boolean",
  "break",
...
static TString MixedLower(const TString& word) {
  TString w;
  w += tolower(word[0]);
...
static TString ToAllUpperCase(const TString& word) {
  TString w;
...
static inline TString LowerMethodName(const MethodDescriptor* method) {
  return MixedLower(method->name());
}

static inline TString MethodPropertiesFieldName(const MethodDescriptor* method) {
  return "METHOD_" + ToAllUpperCase(method->name());
}

static inline TString MethodPropertiesGetterName(const MethodDescriptor* method) {
  return MixedLower("get_" + method->name() + "_method");
}

static inline TString MethodIdFieldName(const MethodDescriptor* method) {
  return "METHODID_" + ToAllUpperCase(method->name());
}

static inline TString MessageFullJavaName(const Descriptor* desc) {
...
static void GrpcSplitStringToIteratorUsing(const TString& full,
                                       const char* delim,
                                       ITR& result) {
  ...
  if (delim[0] != '\0' && delim[1] == '\0') {
    ...
    while (p != end) {
      if (*p == c) {
        ...
        const char* start = p;
        while (++p != end && *p != c);
        *result++ = TString(start, p - start);
      }
    }
    return;
  }

  TString::size_type begin_index, end_index;
  begin_index = full.find_first_not_of(delim);
  while (begin_index != TString::npos) {
...
// TODO(nmittler): Remove once protobuf includes javadoc methods in distribution.
static void GrpcSplitStringUsing(const TString& full,
                             const char* delim,
                             std::vector<TString>* result) {
  std::back_insert_iterator< std::vector<TString> > it(*result);
  GrpcSplitStringToIteratorUsing(full, delim, it);
}

// TODO(nmittler): Remove once protobuf includes javadoc methods in distribution.
static std::vector<TString> GrpcSplit(const TString& full, const char* delim) {
  std::vector<TString> result;
...
// TODO(nmittler): Remove once protobuf includes javadoc methods in distribution.
static TString GrpcEscapeJavadoc(const TString& input) {
  TString result;
  result.reserve(input.size() * 2);

  char prev = '*';

  for (TString::size_type i = 0; i < input.size(); i++) {
    char c = input[i];
    switch (c) {
      case '*':
        // Avoid "/*".
...
static TString GrpcGetCommentsForDescriptor(const DescriptorType* descriptor) {
...
// TODO(nmittler): Remove once protobuf includes javadoc methods in distribution.
static std::vector<TString> GrpcGetDocLines(const TString& comments) {
  if (!comments.empty()) {
    ...
    TString escapedComments = GrpcEscapeJavadoc(comments);

    std::vector<TString> lines = GrpcSplit(escapedComments, "\n");
...
// TODO(nmittler): Remove once protobuf includes javadoc methods in distribution.
template <typename DescriptorType>
static std::vector<TString> GrpcGetDocLinesForDescriptor(const DescriptorType* descriptor) {
...
// TODO(nmittler): Remove once protobuf includes javadoc methods in distribution.
static void GrpcWriteDocComment(Printer* printer, const TString& comments) {
  printer->Print("/**\n");
  std::vector<TString> lines = GrpcGetDocLines(comments);
...
static void GrpcWriteServiceDocComment(Printer* printer,
  ...
  printer->Print("/**\n");
  std::vector<TString> lines = GrpcGetDocLinesForDescriptor(service);
...
void GrpcWriteMethodDocComment(Printer* printer,
  ...
  // (see https://github.com/google/protobuf/issues/1406);
  printer->Print("/**\n");
  std::vector<TString> lines = GrpcGetDocLinesForDescriptor(method);
  GrpcWriteDocCommentBody(printer, lines, true);
  printer->Print(" */\n");
}

static void PrintMethodFields(
    const ServiceDescriptor* service, std::map<TString, TString>* vars,
...
static void PrintStub(
    ...
    std::map<TString, TString>* vars,
    Printer* p, StubType type) {
  const TString service_name = service->name();
  (*vars)["service_name"] = service_name;
  (*vars)["abstract_name"] = service_name + "ImplBase";
  TString stub_name = service_name;
  TString client_name = service_name;
  TString stub_base_class_name = "AbstractStub";
  CallType call_type;
...
static void PrintMethodHandlerClass(const ServiceDescriptor* service,
  ...
  std::stable_sort(sorted_methods.begin(), sorted_methods.end(),
...
void GenerateService(const ServiceDescriptor* service,
  ...
  Printer printer(out, '$');
  TString package_name = ServiceJavaPackage(service->file());
...
TString ServiceJavaPackage(const FileDescriptor* file) {
  TString result = protobuf::compiler::java::ClassName(file);
...
TString ServiceClassName(const ServiceDescriptor* service) {
