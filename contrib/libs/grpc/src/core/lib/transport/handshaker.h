...
class HandshakeManager : public RefCounted<HandshakeManager> {
 public:
  ...
  void Add(RefCountedPtr<Handshaker> handshaker) Y_ABSL_LOCKS_EXCLUDED(mu_);

  /// Shuts down the handshake manager (e.g., to clean up when the operation is
  /// aborted in the middle).
  void Shutdown(grpc_error_handle why) Y_ABSL_LOCKS_EXCLUDED(mu_);
  ...
  /// y_absl::OkStatus(), then handshaking failed and the handshaker has done
 ...
 private:
  ...
  y_absl::InlinedVector<RefCountedPtr<Handshaker>, HANDSHAKERS_INIT_SIZE>
      handshakers_ Y_ABSL_GUARDED_BY(mu_);
  // The index of the handshaker to invoke next and closure to invoke it.
  size_t index_ Y_ABSL_GUARDED_BY(mu_) = 0;
  grpc_closure call_next_handshaker_ Y_ABSL_GUARDED_BY(mu_);
  // The acceptor to call the handshakers with.
  grpc_tcp_server_acceptor* acceptor_ Y_ABSL_GUARDED_BY(mu_);
  // The final callback and user_data to invoke after the last handshaker.
  grpc_closure on_handshake_done_ Y_ABSL_GUARDED_BY(mu_);
  // Handshaker args.
  HandshakerArgs args_ Y_ABSL_GUARDED_BY(mu_);
  // Deadline timer across all handshakers.
  grpc_event_engine::experimental::EventEngine::TaskHandle
      deadline_timer_handle_ Y_ABSL_GUARDED_BY(mu_);
