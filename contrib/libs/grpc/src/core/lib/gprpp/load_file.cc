...
// terminator if true.
y_absl::StatusOr<Slice> LoadFile(TString filename, bool add_null_terminator) {
  unsigned char* contents = nullptr;
  size_t contents_size = 0;
  FILE* file;
  size_t bytes_read = 0;
  y_absl::Status error = y_absl::OkStatus();
  auto sock_cleanup = y_absl::MakeCleanup([&file]() -> void {
    if (file != nullptr) {
      fclose(file);
    }
  });
  ...
  if (file == nullptr) {
    error = y_absl::InternalError(
        y_absl::StrCat("Failed to load file: ", filename,
