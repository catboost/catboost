...
class EventEngineEndpointWrapper {
 public:
  ...
  y_absl::string_view PeerAddress() {
    grpc_core::MutexLock lock(&mu_);
    return peer_address_;
  }

  y_absl::string_view LocalAddress() {
    grpc_core::MutexLock lock(&mu_);
...
void EndpointRead(grpc_endpoint* ep, grpc_slice_buffer* slices,
  ...
  if (!eeep->wrapper->ShutdownRef()) {
    ...
    grpc_core::ExecCtx::Run(DEBUG_LOCATION, cb, y_absl::CancelledError());
    return;
  }
...
y_absl::string_view EndpointGetPeerAddress(grpc_endpoint* ep) {
  auto* eeep =
      reinterpret_cast<EventEngineEndpointWrapper::grpc_event_engine_endpoint*>(
          ep);
...
y_absl::string_view EndpointGetLocalAddress(grpc_endpoint* ep) {
  auto* eeep =
      reinterpret_cast<EventEngineEndpointWrapper::grpc_event_engine_endpoint*>(
          ep);
  ...
  if (fd == nullptr || on_release_fd == nullptr) {
    ...
    *fd = -1;
    eeep->wrapper->TriggerShutdown(
        [fd, on_release_fd](y_absl::StatusOr<int> release_fd) {
          if (release_fd.ok()) {
            *fd = *release_fd;
          }
