...
class PosixSocketWrapper {
 public:
  ...
  // set.
  y_absl::StatusOr<int> SetSocketRcvLowat(int bytes);

  // Set socket to use zerocopy
  y_absl::Status SetSocketZeroCopy();

  // Set socket to non blocking mode
  y_absl::Status SetSocketNonBlocking(int non_blocking);

  // Set socket to close on exec
  y_absl::Status SetSocketCloexec(int close_on_exec);

  // Set socket to reuse old addresses
  y_absl::Status SetSocketReuseAddr(int reuse);

  // Disable nagle algorithm
  y_absl::Status SetSocketLowLatency(int low_latency);

  // Set SO_REUSEPORT
  y_absl::Status SetSocketReusePort(int reuse);
  ...
  y_absl::Status SetSocketNoSigpipeIfPossible();

  // Tries to set IP_PKTINFO if available on this platform. If IP_PKTINFO is not
  // available, returns not OK status.
  y_absl::Status SetSocketIpPktInfoIfPossible();

  // Tries to set IPV6_RECVPKTINFO if available on this platform. If
  // IPV6_RECVPKTINFO is not available, returns not OK status.
  y_absl::Status SetSocketIpv6RecvPktInfoIfPossible();

  // Tries to set the socket's send buffer to given size.
  y_absl::Status SetSocketSndBuf(int buffer_size_bytes);

  // Tries to set the socket's receive buffer to given size.
  y_absl::Status SetSocketRcvBuf(int buffer_size_bytes);

  // Tries to set the socket using a grpc_socket_mutator
  y_absl::Status SetSocketMutator(grpc_fd_usage usage,
                                grpc_socket_mutator* mutator);

  // Extracts the first socket mutator from config if any and applies on the fd.
  y_absl::Status ApplySocketMutatorInOptions(grpc_fd_usage usage,
                                           const PosixTcpOptions& options);

  // Return LocalAddress as EventEngine::ResolvedAddress
  y_absl::StatusOr<EventEngine::ResolvedAddress> LocalAddress();

  // Return PeerAddress as EventEngine::ResolvedAddress
  y_absl::StatusOr<EventEngine::ResolvedAddress> PeerAddress();

  // Return LocalAddress as string
  y_absl::StatusOr<TString> LocalAddressString();

  // Return PeerAddress as string
  y_absl::StatusOr<TString> PeerAddressString();
  ...
  static y_absl::StatusOr<PosixSocketWrapper> CreateDualStackSocket(
  ...
  //
  static y_absl::StatusOr<PosixSocketCreateResult>
