...
y_absl::StatusOr<std::unique_ptr<WakeupFd>> NotSupported() {
  return y_absl::NotFoundError("Wakeup-fd is not supported on this system");
}

namespace {
y_absl::StatusOr<std::unique_ptr<WakeupFd>> (*g_wakeup_fd_fn)() =
...
y_absl::StatusOr<std::unique_ptr<WakeupFd>> CreateWakeupFd() {
