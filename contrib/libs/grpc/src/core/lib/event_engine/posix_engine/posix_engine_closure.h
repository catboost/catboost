...
// The callbacks for Endpoint read and write take an y_absl::Status as
...
class PosixEngineClosure final
 ...
 public:
  ...
  PosixEngineClosure(y_absl::AnyInvocable<void(y_absl::Status)> cb,
                     bool is_permanent)
      : cb_(std::move(cb)),
        is_permanent_(is_permanent),
        status_(y_absl::OkStatus()) {}
  ~PosixEngineClosure() final = default;
  void SetStatus(y_absl::Status status) { status_ = status; }
  void Run() override {
    ...
    if (!is_permanent_) {
      cb_(std::exchange(status_, y_absl::OkStatus()));
      delete this;
    } else {
      cb_(std::exchange(status_, y_absl::OkStatus()));
    }
