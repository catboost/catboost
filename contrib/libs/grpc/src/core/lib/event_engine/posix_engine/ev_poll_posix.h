...
class PollPoller : public PosixEventPoller {
 public:
  ...
  EventHandle* CreateHandle(int fd, y_absl::string_view name,
  ...
  TString Name() override { return "poll"; }
  void Kick() override;
 ...
 private:
  ...
  bool was_kicked_ Y_ABSL_GUARDED_BY(mu_);
  bool was_kicked_ext_ Y_ABSL_GUARDED_BY(mu_);
  int num_poll_handles_ Y_ABSL_GUARDED_BY(mu_);
  PollEventHandle* poll_handles_list_head_ Y_ABSL_GUARDED_BY(mu_) = nullptr;
