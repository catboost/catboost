...
void AsyncConnect::Start(EventEngine::Duration timeout) {
  on_writable_ = PosixEngineClosure::ToPermanentClosure(
      [this](y_absl::Status status) { OnWritable(std::move(status)); });
  alarm_handle_ = engine_->RunAfter(timeout, [this]() {
    OnTimeoutExpired(y_absl::DeadlineExceededError("connect() timed out"));
  });
...
void AsyncConnect::OnTimeoutExpired(y_absl::Status status) {
  bool done = false;
  {
...
void AsyncConnect::OnWritable(y_absl::Status status)
  ...
  int consumed_refs = 1;
  EventHandle* fd;
  y_absl::StatusOr<std::unique_ptr<EventEngine::Endpoint>> ep;
  ...
  if (fd->IsHandleShutdown() && status.ok()) {
    if (!connect_cancelled) {
      ...
      status = y_absl::DeadlineExceededError("connect() timed out");
    } else {
      // This can happen if the connection was cancelled while the OnWritable
      // just started executing.
      status = y_absl::FailedPreconditionError("Connection cancelled");
    }
  }
  mu_.Unlock();
  ...
  auto on_writable_finish = y_absl::MakeCleanup([&]() -> void {
    mu_.AssertHeld();
    if (!connect_cancelled) {
    ...
    if (!status.ok()) {
      ep = y_absl::UnknownError(
          y_absl::StrCat("Failed to connect to remote host: ", status.message()));
    }
  ...
  if (err < 0) {
    status = y_absl::FailedPreconditionError(
        y_absl::StrCat("getsockopt: ", std::strerror(errno)));
    return;
  }
  ...
  switch (so_error) {
    ...
    case ECONNREFUSED:
      ...
      status = y_absl::FailedPreconditionError(std::strerror(so_error));
      break;
    default:
      // We don't really know which syscall triggered the problem here, so
      // punt by reporting getsockopt().
      status = y_absl::FailedPreconditionError(
          y_absl::StrCat("getsockopt(SO_ERROR): ", std::strerror(so_error)));
      break;
  }
}
...
EventEngine::ConnectionHandle PosixEventEngine::ConnectInternal(
  ...
  if (saved_errno != EWOULDBLOCK && saved_errno != EINPROGRESS) {
    ...
    Run([on_connect = std::move(on_connect),
         ep = y_absl::FailedPreconditionError(
             y_absl::StrCat("connect failed: ", "addr: ", addr_uri.value(),
...
void PosixEnginePollerManager::Run(y_absl::AnyInvocable<void()> cb) {
  if (executor_ != nullptr) {
...
bool PosixEventEngine::CancelConnect(EventEngine::ConnectionHandle handle) {
  ...
  if (connection_cancel_success) {
    ...
    ac->fd_->ShutdownHandle(
        y_absl::FailedPreconditionError("Connection cancelled"));
  }
...
EventEngine::ConnectionHandle PosixEventEngine::Connect(
  ...
  y_absl::StatusOr<PosixSocketWrapper::PosixSocketCreateResult> socket =
...
y_absl::StatusOr<std::unique_ptr<PosixListenerWithFdSupport>>
