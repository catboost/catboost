...
class AsyncConnect {
 public:
  AsyncConnect(EventEngine::OnConnectCallback on_connect,
               ...
               TString resolved_addr_str, int64_t connection_handle)
 ...
 private:
  ...
  void OnTimeoutExpired(y_absl::Status status);

  void OnWritable(y_absl::Status status) Y_ABSL_NO_THREAD_SAFETY_ANALYSIS;
...
class PosixEnginePollerManager
 ...
 public:
  ...
  void Run(y_absl::AnyInvocable<void()>) override;
...
class PosixEventEngine final : public PosixEventEngineWithFdSupport,
 ...
 private:
  ...
  struct ConnectionShard {
    grpc_core::Mutex mu;
    y_absl::flat_hash_map<int64_t, AsyncConnect*> pending_connections
