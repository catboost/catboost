...
class AnyInvocableClosure : public EventEngine::Closure {
 public:
  explicit AnyInvocableClosure(y_absl::AnyInvocable<void()> cb)
      : cb_(std::move(cb)) {}
  void Run() override { cb_(); }

 private:
  y_absl::AnyInvocable<void()> cb_;
};
...
class SelfDeletingClosure : public EventEngine::Closure {
 ...
 private:
  explicit SelfDeletingClosure(y_absl::AnyInvocable<void()> cb,
                               y_absl::AnyInvocable<void()> dest_cb)
      : cb_(std::move(cb)), dest_cb_(std::move(dest_cb)) {}
  y_absl::AnyInvocable<void()> cb_;
  y_absl::AnyInvocable<void()> dest_cb_;
};
