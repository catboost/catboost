 ...
 protected:
  ...
  class Flusher {
   public:
    ...
    void Complete(grpc_transport_stream_op_batch* batch) {
      call_closures_.Add(batch->on_complete, y_absl::OkStatus(),
                         "Flusher::Complete");
    }
  ...
  class SendMessage {
   ...
   private:
    ...
    void OnComplete(y_absl::Status status);

    BaseCallData* const base_;
    State state_ = State::kInitial;
    Interceptor* const interceptor_;
    y_absl::optional<PipeSender<MessageHandle>::PushType> push_;
    y_absl::optional<PipeReceiverNextType<MessageHandle>> next_;
    CapturedBatch batch_;
    grpc_closure* intercepted_on_complete_;
    grpc_closure on_complete_ =
        MakeMemberClosure<SendMessage, &SendMessage::OnComplete>(this);
    y_absl::Status completed_status_;
  };
  ...
  class ReceiveMessage {
   ...
   private:
    ...
    y_absl::optional<SliceBuffer>* intercepted_slice_buffer_;
  ...
  virtual y_absl::string_view ClientOrServerString() const = 0;
...
class ClientCallData : public BaseCallData {
 ...
 private:
  ...
  y_absl::string_view ClientOrServerString() const override { return "CLI"; }
...
struct CallDataFilterWithFlagsMethods {
  static y_absl::Status InitCallElem(grpc_call_element* elem,
...
struct ChannelFilterWithFlagsMethods {
  static y_absl::Status InitChannelElem(grpc_channel_element* elem,
