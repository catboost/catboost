...
class FakeActivity final : public Activity {
 public:
  ...
  void Run(y_absl::FunctionRef<void()> f) {
...
y_absl::Status StatusFromMetadata(const ServerMetadata& md) {
...
BaseCallData::BaseCallData(
    ...
    y_absl::FunctionRef<Interceptor*()> make_send_interceptor,
    y_absl::FunctionRef<Interceptor*()> make_recv_interceptor)
...
TString BaseCallData::LogTag() const {
  return y_absl::StrCat(
      ...
      y_absl::Hex(reinterpret_cast<uintptr_t>(elem_), y_absl::kZeroPad8), "]");
}
...
BaseCallData::Flusher::~Flusher() {
  ...
  for (size_t i = 1; i < release_.size(); i++) {
    ...
    call_closures_.Add(&batch->handler_private.closure, y_absl::OkStatus(),
                       "flusher_batch");
  }
...
void BaseCallData::SendMessage::OnComplete(y_absl::Status status) {
...
void BaseCallData::SendMessage::Done(const ServerMetadata& metadata,
  ...
  switch (state_) {
    ...
    case State::kGotBatch: {
      ...
      batch_.CancelWith(
          y_absl::Status(
              static_cast<y_absl::StatusCode>(metadata.get(GrpcStatusMetadata())
...
void BaseCallData::SendMessage::WakeInsideCombiner(Flusher* flusher,
  ...
  if (grpc_trace_channel.enabled()) {
    gpr_log(GPR_INFO, "%s SendMessage.WakeInsideCombiner st=%s%s",
            base_->LogTag().c_str(), StateString(state_),
            state_ == State::kBatchCompleted
                ? y_absl::StrCat(" status=", completed_status_.ToString()).c_str()
                : "");
  }
  switch (state_) {
    case State::kInitial:
    ...
    case State::kPushedToPipe: {
      ...
      if (auto* p = r_push.value_if_ready()) {
        ...
        GPR_ASSERT(!*p);
        state_ = State::kCancelled;
        batch_.CancelWith(y_absl::CancelledError(), flusher);
        break;
      }
...
void BaseCallData::ReceiveMessage::OnComplete(y_absl::Status status) {
  if (grpc_trace_channel.enabled()) {
    ...
    case State::kPushedToPipe: {
      ...
      if (auto* p = r_next.value_if_ready()) {
        ...
        if (p->has_value()) {
          ...
          *intercepted_slice_buffer_ = y_absl::nullopt;
          *intercepted_flags_ = 0;
          state_ = State::kCancelled;
          flusher->AddClosure(
              std::exchange(intercepted_on_complete_, nullptr),
              p->cancelled() ? y_absl::CancelledError() : y_absl::OkStatus(),
              "recv_message");
        }
        if (grpc_trace_channel.enabled()) {
          gpr_log(GPR_INFO,
...
struct ClientCallData::RecvInitialMetadata final {
  ...
  y_absl::optional<PipeSender<ServerMetadataHandle>::PushType> metadata_push_;
  y_absl::optional<PipeReceiverNextType<ServerMetadataHandle>> metadata_next_;
 ...
 public:
          ...
          if (self_->recv_trailing_state_ == RecvTrailingState::kComplete) {
            ...
            flusher_->AddClosure(
                std::exchange(self_->original_recv_trailing_metadata_ready_,
                              nullptr),
                y_absl::OkStatus(), "wake_inside_combiner:recv_trailing_ready:1");
            if (self_->recv_initial_metadata_ != nullptr) {
              switch (self_->recv_initial_metadata_->state) {
                ...
                case RecvInitialMetadata::
                    kRespondedToTrailingMetadataPriorToHook:
                case RecvInitialMetadata::kRespondedButNeedToClosePipe:
                  Crash(y_absl::StrFormat("ILLEGAL STATE: %s",
      ...
      case SendInitialState::kCancelled:
        ...
        if (self_->recv_trailing_state_ == RecvTrailingState::kComplete) {
          ...
          flusher_->AddClosure(
              std::exchange(self_->original_recv_trailing_metadata_ready_,
                            nullptr),
              y_absl::OkStatus(), "wake_inside_combiner:recv_trailing_ready:2");
        }
        break;
    }
  }
  ...
  ~PollContext() {
    ...
    if (repoll_) {
      ...
      flusher_->AddClosure(p, y_absl::OkStatus(), "re-poll");
    }
  }
...
TString ClientCallData::DebugString() const {
  std::vector<y_absl::string_view> captured;
  ...
  return y_absl::StrCat(
      ...
      y_absl::StrJoin(captured, ","), "}",
      server_initial_metadata_pipe() == nullptr
          ? ""
          : y_absl::StrCat(" recv_initial_metadata=",
                         RecvInitialMetadata::StateString(
...
void ClientCallData::StartBatch(grpc_transport_stream_op_batch* b) {
  ...
  if (batch.is_captured()) {
    if (!is_last()) {
      ...
      batch.CancelWith(y_absl::CancelledError(), &flusher);
    }
  }
}
...
void ServerCallData::StartBatch(grpc_transport_stream_op_batch* b) {
  ...
  if (send_initial_metadata_ != nullptr && batch->send_initial_metadata) {
    switch (send_initial_metadata_->state) {
      ...
      case SendInitialMetadata::kCancelled:
        batch.CancelWith(
            cancelled_error_.ok() ? y_absl::CancelledError() : cancelled_error_,
            &flusher);
        break;
  ...
  if (batch.is_captured() && batch->send_trailing_metadata) {
    switch (send_trailing_state_) {
      ...
      case SendTrailingState::kForwarded:
        Crash(
            y_absl::StrFormat("ILLEGAL STATE: %s",
...
TString ServerCallData::DebugString() const {
  std::vector<y_absl::string_view> captured;
  if (send_message() != nullptr && send_message()->HaveCapturedBatch()) {
    captured.push_back("send_message");
  }
...
void ServerCallData::WakeInsideCombiner(Flusher* flusher) {
  ...
  if (std::exchange(forward_recv_initial_metadata_callback_, false)) {
    if (auto* closure =
            std::exchange(original_recv_initial_metadata_ready_, nullptr)) {
      flusher->AddClosure(closure, y_absl::OkStatus(),
