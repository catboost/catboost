...
class Poll {
 ...
 private:
  ...
  // Why not optional<T>?
  //
  // We have cases where we want to return y_absl::nullopt{} from a promise, and
  // have that upgraded to a Poll<y_absl::nullopt_t> prior to a cast to some
  // Poll<optional<T>>.
  //
