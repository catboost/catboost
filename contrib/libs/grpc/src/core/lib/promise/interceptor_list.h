...
class InterceptorList {
 private:
  ...
  class Map {
   public:
    ...
    // resolve.
    virtual Poll<y_absl::optional<T>> PollOnce(void* memory) = 0;
    virtual ~Map() = default;
 ...
 public:
  ...
  class RunPromise {
   ...
   private:
    TString DebugString() const {
      if (is_immediately_resolved_) {
        return y_absl::StrFormat("Result:has_value:%d", result_.has_value());
      } else {
        return y_absl::StrCat(
            ...
            async_resolution_.current_factory == nullptr
                ...
                : ([p = async_resolution_.current_factory->from()]() {
                    return y_absl::StrCat(p.file(), ":", p.line());
                  })()
                      .c_str());
      }
    }
  ...
  RunPromise Run(y_absl::optional<T> initial_value) {
 ...
 private:
  ...
  class MapImpl final : public Map {
   public:
    ...
    Poll<y_absl::optional<T>> PollOnce(void* memory) override {
      return poll_cast<y_absl::optional<T>>((*static_cast<Promise*>(memory))());
    }
