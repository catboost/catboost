...
y_absl::Status DoesNotConformTo(y_absl::string_view x, const char* err_desc) {
  return y_absl::InternalError(y_absl::StrCat(err_desc, ": ", x, " (hex ",
                                          y_absl::BytesToHexString(x), ")"));
}

y_absl::Status ConformsTo(y_absl::string_view x, const BitSet<256>& legal_bits,
                        const char* err_desc) {
  for (uint8_t c : x) {
...
y_absl::Status ValidateHeaderKeyIsLegal(y_absl::string_view key) {
  if (key.empty()) {
    return y_absl::InternalError("Metadata keys cannot be zero length");
  }
