...
Channel::Channel(bool is_client, bool is_promising, TString target,
...
y_absl::StatusOr<RefCountedPtr<Channel>> Channel::CreateWithBuilder(
  ...
  return RefCountedPtr<Channel>(new Channel(
      ...
      builder->IsPromising(), TString(builder->target()), channel_args,
...
y_absl::StatusOr<RefCountedPtr<Channel>> Channel::Create(
  ...
  if (grpc_channel_stack_type_is_client(channel_stack_type)) {
    ...
    if (args.GetBool(GRPC_ARG_ENABLE_CHANNELZ)
      ...
      TString channelz_node_target{target == nullptr ? "unknown" : target};
