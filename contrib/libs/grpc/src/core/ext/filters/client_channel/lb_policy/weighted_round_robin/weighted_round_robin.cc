...
constexpr y_absl::string_view kWeightedRoundRobin =
...
class WeightedRoundRobinConfig : public LoadBalancingPolicy::Config {
 public:
  ...
  y_absl::string_view name() const override { return kWeightedRoundRobin; }
...
class WeightedRoundRobin : public LoadBalancingPolicy {
 ...
 private:
  ...
  class AddressWeight : public RefCounted<AddressWeight> {
   public:
    AddressWeight(RefCountedPtr<WeightedRoundRobin> wrr, TString key)
   ...
   private:
    ...
    Mutex mu_;
    float weight_ Y_ABSL_GUARDED_BY(&mu_) = 0;
    Timestamp non_empty_since_ Y_ABSL_GUARDED_BY(&mu_) = Timestamp::InfFuture();
    Timestamp last_update_time_ Y_ABSL_GUARDED_BY(&mu_) = Timestamp::InfPast();
  };
  ...
  class Picker : public SubchannelPicker {
   ...
   private:
    ...
    void BuildSchedulerAndStartTimerLocked()
        Y_ABSL_EXCLUSIVE_LOCKS_REQUIRED(&timer_mu_);
    ...
    std::shared_ptr<StaticStrideScheduler> scheduler_
        Y_ABSL_GUARDED_BY(&scheduler_mu_);

    Mutex timer_mu_ Y_ABSL_ACQUIRED_BEFORE(&scheduler_mu_);
    y_absl::optional<grpc_event_engine::experimental::EventEngine::TaskHandle>
        timer_handle_ Y_ABSL_GUARDED_BY(&timer_mu_);
  ...
  std::map<TString, AddressWeight*, std::less<>> address_weight_map_
      Y_ABSL_GUARDED_BY(&address_weight_map_mu_);
  ...
  // Accessed by picker.
  std::atomic<uint32_t> scheduler_state_{y_absl::Uniform<uint32_t>(bit_gen_)};
};
...
void WeightedRoundRobin::WeightedRoundRobinSubchannelList::
    UpdateStateCountersLocked(y_absl::optional<grpc_connectivity_state> old_state,
                              grpc_connectivity_state new_state) {
...
void WeightedRoundRobin::WeightedRoundRobinSubchannelList::
    MaybeUpdateAggregatedConnectivityStateLocked(y_absl::Status status_for_tf) {
  WeightedRoundRobin* p = static_cast<WeightedRoundRobin*>(policy());
