 ...
 private:
  ...
  class Picker : public SubchannelPicker {
   ...
   private:
    ...
    y_absl::optional<LoadBalancingPolicy::PickResult> PickOverridenHost(
        y_absl::string_view override_host);
  ...
  class SubchannelEntry {
   ...
   private:
    y_absl::variant<SubchannelWrapper*, RefCountedPtr<SubchannelWrapper>>
        subchannel_;
  ...
  y_absl::StatusOr<ServerAddressList> UpdateAddressMap(
  ...
  void UnsetSubchannel(y_absl::string_view key, SubchannelWrapper* subchannel);

  RefCountedPtr<SubchannelWrapper> GetSubchannelByAddress(
      y_absl::string_view address, XdsHealthStatusSet overriden_health_statuses);

  void OnSubchannelConnectivityStateChange(y_absl::string_view subchannel_key)
      Y_ABSL_NO_THREAD_SAFETY_ANALYSIS;  // Called from within the worker
  ...
  std::map<TString, SubchannelEntry, std::less<>> subchannel_map_
      Y_ABSL_GUARDED_BY(subchannel_map_mu_);
};
...
y_absl::optional<LoadBalancingPolicy::PickResult>
XdsOverrideHostLb::Picker::PickOverridenHost(y_absl::string_view override_host) {
...
y_absl::Status XdsOverrideHostLb::UpdateLocked(UpdateArgs args) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_lb_xds_override_host_trace)) {
  ...
  if (config_ == nullptr) {
    return y_absl::InvalidArgumentError("Missing policy config");
  }
...
y_absl::StatusOr<ServerAddressList> XdsOverrideHostLb::UpdateAddressMap(
    y_absl::StatusOr<ServerAddressList> addresses) {
  if (!addresses.ok()) {
    return addresses;
  }
  ServerAddressList return_value;
  std::map<const TString, XdsHealthStatus> addresses_for_map;
...
void XdsOverrideHostLb::UnsetSubchannel(y_absl::string_view key,
...
void XdsOverrideHostLb::OnSubchannelConnectivityStateChange(
    y_absl::string_view subchannel_key) {
...
y_absl::string_view XdsOverrideHostLb::Helper::GetAuthority() {
...
XdsOverrideHostLb::SubchannelWrapper::SubchannelWrapper(
    ...
    RefCountedPtr<XdsOverrideHostLb> policy, y_absl::string_view key)
