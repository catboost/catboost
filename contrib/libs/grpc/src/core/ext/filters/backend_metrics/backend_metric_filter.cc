...
y_absl::optional<TString> BackendMetricFilter::MaybeSerializeBackendMetrics(
    BackendMetricProvider* provider) const {
  if (provider == nullptr) return y_absl::nullopt;
...
y_absl::StatusOr<BackendMetricFilter> BackendMetricFilter::Create(
...
ArenaPromise<ServerMetadataHandle> BackendMetricFilter::MakeCallPromise(
  ...
  return ArenaPromise<ServerMetadataHandle>(Map(
      ...
      [this](ServerMetadataHandle trailing_metadata) {
        ...
        y_absl::optional<TString> serialized = MaybeSerializeBackendMetrics(
