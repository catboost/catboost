...
CertificateValidationContextParse(
  ...
  for (size_t i = 0; i < len; ++i) {
    ValidationErrors::ScopedField field(
        errors, y_absl::StrCat(".match_subject_alt_names[", i, "]"));
...
y_absl::StatusOr<Json> ParseProtobufStructToJson(
    const XdsResourceType::DecodeContext& context,
  ...
  if (json_size == static_cast<size_t>(-1)) {
    return y_absl::InvalidArgumentError(
        y_absl::StrCat("error encoding google::Protobuf::Struct as JSON: ",
...
y_absl::optional<XdsExtension> ExtractXdsExtension(
  ...
  auto strip_type_prefix = [&]() {
    ...
    if (pos == y_absl::string_view::npos || pos == extension.type.size() - 1) {
      errors->AddError(y_absl::StrCat("invalid value \"", extension.type, "\""));
    } else {
  ...
  if (!strip_type_prefix()) return y_absl::nullopt;
  extension.validation_fields.emplace_back(
      errors, y_absl::StrCat(".value[", extension.type, "]"));
  y_absl::string_view any_value = UpbStringToAbsl(google_protobuf_Any_value(any));
