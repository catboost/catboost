...
class XdsServerConfigFetcher::ListenerWatcher::FilterChainMatchManager::
 ...
 public:
  ...
  y_absl::StatusOr<CallConfig> GetCallConfig(
...
void XdsServerConfigFetcher::ListenerWatcher::OnResourceChanged(
  ...
  auto* tcp_listener =
      y_absl::get_if<XdsListenerResource::TcpListener>(&listener.listener);
  if (tcp_listener == nullptr) {
    MutexLock lock(&mu_);
    OnFatalError(
        y_absl::FailedPreconditionError("LDS resource is not a TCP listener"));
    return;
  }
...
void XdsServerConfigFetcher::ListenerWatcher::FilterChainMatchManager::
    ...
    for (const auto& source_type : destination_ip.source_types_array) {
      for (const auto& source_ip : source_type) {
        for (const auto& source_port_pair : source_ip.ports_map) {
          ...
          const auto* rds_name = y_absl::get_if<TString>(
  ...
  if (default_filter_chain_.has_value()) {
    ...
    const auto* rds_name = y_absl::get_if<TString>(&hcm.route_config);
    if (rds_name != nullptr) resource_names.insert(*rds_name);
...
XdsServerConfigFetcher::ListenerWatcher::FilterChainMatchManager::
  ...
  if (metadata->get_pointer(HttpAuthorityMetadata()) == nullptr) {
    return y_absl::InternalError("no authority found");
  }
  ...
  if (!vhost_index.has_value()) {
    return y_absl::UnavailableError(
        y_absl::StrCat("could not find VirtualHost for ", authority,
  ...
  if (route_index.has_value()) {
    ...
    if (route.unsupported_action) {
      return y_absl::UnavailableError("matching route has unsupported action");
    }
  ...
  return y_absl::UnavailableError("no route matched");
}
