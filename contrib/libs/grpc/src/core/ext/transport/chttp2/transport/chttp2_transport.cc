...
grpc_chttp2_stream::~grpc_chttp2_stream() {
  ...
  for (int i = 0; i < STREAM_LIST_COUNT; i++) {
    if (GPR_UNLIKELY(included.is_set(i))) {
      grpc_core::Crash(y_absl::StrFormat("%s stream %d still included in list %d",
  ...
  grpc_core::ExecCtx::Run(DEBUG_LOCATION, destroy_stream_arg, y_absl::OkStatus());
}
...
static void set_write_state(grpc_chttp2_transport* t,
  ...
  if (st == GRPC_CHTTP2_WRITE_STATE_IDLE) {
    ...
    if (!t->close_transport_on_writes_finished.ok()) {
      ...
      t->close_transport_on_writes_finished = y_absl::OkStatus();
...
static void write_action_begin_locked(void* gt,
  ...
  if (r.writing) {
    ...
    write_action(t, y_absl::OkStatus());
...
static void null_then_sched_closure(grpc_closure** closure) {
  ...
  *closure = nullptr;
  ...
  // forcing a race.
  grpc_core::ExecCtx::Run(DEBUG_LOCATION, c, y_absl::OkStatus());
}
...
void grpc_chttp2_complete_closure_step(grpc_chttp2_transport* t,
  ...
  *pclosure = nullptr;
  ...
  if (!error.ok()) {
    ...
    if (cl_err.ok()) {
      cl_err = GRPC_ERROR_CREATE(y_absl::StrCat(
...
static void perform_stream_op_locked(void* stream_op,
  ...
  if (on_complete != nullptr) {
    grpc_chttp2_complete_closure_step(t, s, &on_complete, y_absl::OkStatus(),
                                      "op->on_complete");
  }
        ...
        while (true) {
          ...
          if (grpc_http_trace.enabled()) {
            gpr_log(GPR_DEBUG, "Deframe data frame: %s",
                    grpc_core::PollToString(r, [](y_absl::Status r) {
                      return r.ToString();
                    }).c_str());
          }
          if (r.pending()) {
...
static grpc_error_handle try_http_parsing(grpc_chttp2_transport* t) {
  ...
  if (parse_error.ok() &&
      (parse_error = grpc_http_parser_eof(&parser)) == y_absl::OkStatus()) {
...
static void read_action_locked(void* tp, grpc_error_handle error) {
  ...
  if (t->closed_with_error.ok()) {
    size_t i = 0;
    grpc_error_handle errors[3] = {error, y_absl::OkStatus(), y_absl::OkStatus()};
    for (; i < t->read_buffer.count && errors[1] == y_absl::OkStatus(); i++) {
      errors[1] = grpc_chttp2_perform_read(t, t->read_buffer.slices[i]);
    }
    if (errors[1] != y_absl::OkStatus()) {
    ...
    t->memory_owner.PostReclaimer(
        ...
        [t](y_absl::optional<grpc_core::ReclamationSweep> sweep) {
          if (sweep.has_value()) {
            ...
            t->combiner->Run(&t->benign_reclaimer_locked, y_absl::OkStatus());
          } else {
