...
TString MakeFrameTypeString(y_absl::string_view frame_type, uint8_t flags,
                                std::initializer_list<KnownFlag> known_flags) {
  TString result(frame_type);
  for (const KnownFlag& known_flag : known_flags) {
    if (flags & known_flag.flag) {
      y_absl::StrAppend(&result, ":", known_flag.name);
      flags &= ~known_flag.flag;
    }
  }
  if (flags != 0) {
    y_absl::StrAppend(&result, ":UNKNOWN_FLAGS=0x",
                    y_absl::Hex(flags, y_absl::kZeroPad2));
  }
  return result;
}

TString FrameTypeString(uint8_t frame_type, uint8_t flags) {
  switch (frame_type) {
    ...
    default:
      return MakeFrameTypeString(
          y_absl::StrCat("UNKNOWN_FRAME_TYPE_", static_cast<int>(frame_type)),
          flags, {});
  }
}
}  // namespace
...
grpc_error_handle grpc_chttp2_perform_read(grpc_chttp2_transport* t,
  ...
  if (cur == end) return y_absl::OkStatus();
...
static grpc_error_handle parse_frame_slice(grpc_chttp2_transport* t,
  ...
  if (grpc_error_get_int(err, grpc_core::StatusIntProperty::kStreamId,
    ...
    if (s) {
      grpc_chttp2_cancel_stream(t, s, std::exchange(err, y_absl::OkStatus()));
    }
  }
  return err;
}
...
static void force_client_rst_stream(void* sp, grpc_error_handle /*error*/) {
  ...
  if (!s->write_closed) {
    ...
    grpc_chttp2_mark_stream_closed(t, s, true, true, y_absl::OkStatus());
  }
  ...
  if (is_last) {
    ...
    if (s != nullptr) {
      ...
      if (parser->is_eof()) {
        ...
        grpc_chttp2_mark_stream_closed(t, s, true, false, y_absl::OkStatus());
      }
    }
    parser->FinishFrame();
  }
