...
class Encoder {
 public:
  ...
  void EncodeAlwaysIndexed(uint32_t* index, y_absl::string_view key, Slice value,
...
class Compressor<MetadataTrait, NoCompressionCompressor> {
 public:
  void EncodeWith(MetadataTrait, const typename MetadataTrait::ValueType& value,
    ...
    if (y_absl::EndsWith(MetadataTrait::key(), "-bin")) {
 ...
 public:
  void EncodeWith(MetadataTrait, const typename MetadataTrait::ValueType& value,
    ...
    if (value != known_value) {
      gpr_log(GPR_ERROR, "%s",
              y_absl::StrCat("Not encoding bad ", MetadataTrait::key(), " header")
                  .c_str());
      return;
    }
...
class SliceIndex {
 public:
  void EmitTo(y_absl::string_view key, const Slice& value, Encoder* encoder);
...
class TimeoutCompressorImpl {
 public:
  void EncodeWith(y_absl::string_view key, Timestamp deadline, Encoder* encoder);
