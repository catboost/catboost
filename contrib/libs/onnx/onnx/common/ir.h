...
public:
  ...
  Value* setUniqueName(TString name) {
...
class OpSetID final {
  ...
  public:
    ...
    explicit OpSetID(const TString& domain, int64_t version)
...
public:
  ...
  const std::vector<TString>& initializer_names() {
    return initializer_names_;
  }
  std::vector<Tensor>::const_iterator getInitializer(const TString& name) {
