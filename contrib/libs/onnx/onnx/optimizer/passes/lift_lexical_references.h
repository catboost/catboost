...
struct LiftLexicalReferences : public FullGraphBasedPass {
  ...
  TString getPassName() const override {
  ...
  using ValueTable = std::unordered_map<TString, Value*>;
  ...
  struct Environment {
    ...
    Value* findInThisFrame(const TString& name) {
    ...
    Value* findInParentFrame(const TString& name) {
      return next ? next->findInAnyFrame(name) : nullptr;
    }

    Value* findInAnyFrame(const TString& name) {
    ...
    void setVar(const TString& name, Value* value) {
  ...
  std::set<TString> liftReferences(Graph* g) {
    std::set<TString> unresolved_references;
    pushFrame();
    ...
    for (auto* n : g->nodes()) {
      ...
      std::set<TString> local_unresolved;
      ...
      std::vector<TString> control_inputs;
  ...
  std::shared_ptr<PostPassAnalysis> runPass(Graph& graph) override {
    ...
    if (unresolved.size()) {
      TString errmsg = "Unresolved value references: ";
