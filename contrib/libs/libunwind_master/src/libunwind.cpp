...
void _unw_remove_dynamic_fde(unw_word_t fde) {
  ...
  const char *message = CFI_Parser<LocalAddressSpace>::decodeFDE(
                          ...
                          (LocalAddressSpace::pint_t) fde, &fdeInfo, &cieInfo);
  if (message == NULL) {
    DwarfFDECache<LocalAddressSpace>::removeAllIn(fdeInfo.fdeStart);
  } else {
    _LIBUNWIND_DEBUG_LOG("_unw_remove_dynamic_fde: bad fde: %s", message);
  }
}
...
#define LIBUNW_TSAN_IS_ON 0
#if defined(__has_feature)
    #if __has_feature(thread_sanitizer)
        #undef LIBUNW_TSAN_IS_ON
        #define LIBUNW_TSAN_IS_ON 1
    #endif
#endif

#if (LIBUNW_TSAN_IS_ON)

// Disable these hooks when thread sanitizer is enabled
_LIBUNWIND_HIDDEN
bool logAPIs() {
    return false;
}
