...
const char *CFI_Parser<A>::decodeFDE(A &addressSpace, pint_t fdeStart,
  ...
  if (ciePointer == 0) {
    // this is a CIE not an FDE, so skip it and parse following FDE
    return decodeFDE(addressSpace, p + cfiLength, fdeInfo, cieInfo);
  }
...
bool CFI_Parser<A>::parseInstructions(A &addressSpace, pint_t instructions,
  ...
  while ((p < instructionsEnd) && (codeOffset < pcoffset)) {
    ...
    case DW_CFA_remember_state:
      ...
      entry = (PrologInfoStackEntry *)AllocPage(sizeof(PrologInfoStackEntry));
      if (entry != NULL) {
