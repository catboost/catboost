...
struct ___bounded_iter {
  static_assert(__libcpp_is_contiguous_iterator<_Iterator>::value,
                "Only contiguous iterators can be adapted by ___bounded_iter.");
...
private:
  ...
  _LIBCPP_CONSTEXPR_SINCE_CXX14 explicit ___bounded_iter(_Iterator __current, _Iterator __begin, _Iterator __end)
...
public:
  ...
  operator<=>(___bounded_iter const& __x, ___bounded_iter const& __y) noexcept {
