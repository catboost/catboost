...
#ifdef Y_ABSL_SWISSTABLE_ASSERT
#error Y_ABSL_SWISSTABLE_ASSERT cannot be directly set
#else
// We use this macro for assertions that users may see when the table is in an
// invalid state that sanitizers may help diagnose.
#define Y_ABSL_SWISSTABLE_ASSERT(CONDITION) \
...
class probe_seq {
 public:
  ...
  probe_seq(size_t hash, size_t mask) {
    Y_ABSL_SWISSTABLE_ASSERT(((mask + 1) & mask) == 0 && "not a mask");
    mask_ = mask;
    offset_ = hash & mask_;
  }
 ...
 public:
  explicit BitMask(T mask) : Base(mask) {
    if (Shift == 3 && !NullifyBitsOnIteration) {
      Y_ABSL_SWISSTABLE_ASSERT(this->mask_ == (this->mask_ & kMsbs8Bytes));
    }
  }
  ...
  explicit RawHashSetLayout(size_t capacity, size_t slot_align, bool has_infoz)
      : capacity_(capacity),
        ...
        slot_offset_(
            ...
            (~slot_align + 1)) {
    Y_ABSL_SWISSTABLE_ASSERT(IsValidCapacity(capacity));
  }
...
class CommonFields : public CommonFieldsGenerationInfo {
 public:
  ...
  void increment_size() {
    Y_ABSL_SWISSTABLE_ASSERT(size() < capacity());
    size_ += size_t{1} << HasInfozShift();
  }
  void decrement_size() {
    Y_ABSL_SWISSTABLE_ASSERT(size() > 0);
    size_ -= size_t{1} << HasInfozShift();
  }
  ...
  void set_capacity(size_t c) {
    ...
    Y_ABSL_SWISSTABLE_ASSERT(c == 0 || IsValidCapacity(c) ||
  ...
  void set_infoz(HashtablezInfoHandle infoz) {
    Y_ABSL_SWISSTABLE_ASSERT(has_infoz());
 ...
 private:
  ...
  void AssertInSooMode() const {
    Y_ABSL_SWISSTABLE_ASSERT(capacity() == SooCapacity());
    Y_ABSL_SWISSTABLE_ASSERT(!has_infoz());
  }
...
inline size_t NextCapacity(size_t n) {
  Y_ABSL_SWISSTABLE_ASSERT(IsValidCapacity(n) || n == 0);
  return n * 2 + 1;
}
...
inline FindInfo find_first_non_full(const CommonFields& common, size_t hash) {
  ...
  while (true) {
    ...
    seq.next();
    Y_ABSL_SWISSTABLE_ASSERT(seq.index() <= common.capacity() && "full table!");
  }
}
...
inline void DoSanitizeOnSetCtrl(const CommonFields& c, size_t i, ctrl_t h,
                                size_t slot_size) {
  Y_ABSL_SWISSTABLE_ASSERT(i < c.capacity());
...
inline void SetCtrlInSingleGroupTable(const CommonFields& c, size_t i, ctrl_t h,
                                      size_t slot_size) {
  Y_ABSL_SWISSTABLE_ASSERT(is_single_group(c.capacity()));
  DoSanitizeOnSetCtrl(c, i, h, slot_size);
  ctrl_t* ctrl = c.control();
  ctrl[i] = h;
...
Y_ABSL_ATTRIBUTE_ALWAYS_INLINE inline void IterateOverFullSlots(
  ...
  if (is_small(cap)) {
    ...
    Y_ABSL_SWISSTABLE_ASSERT(cap <= GroupPortableImpl::kWidth &&
  ...
  while (remaining != 0) {
    for (uint32_t i : GroupFullEmptyOrDeleted(ctrl).MaskFull()) {
      Y_ABSL_SWISSTABLE_ASSERT(IsFull(ctrl[i]) &&
  ...
  Y_ABSL_SWISSTABLE_ASSERT(original_size_for_assert >= c.size() &&
                         "hash table was modified unexpectedly");
}
...
class HashSetResizeHelper {
 public:
  ...
  ctrl_t* old_ctrl() const {
    Y_ABSL_SWISSTABLE_ASSERT(!was_soo_);
  ...
  Y_ABSL_ATTRIBUTE_NOINLINE bool InitializeSlots(CommonFields& c, Alloc alloc,
                                               ...
                                               size_t key_size,
                                               size_t value_size) {
    Y_ABSL_SWISSTABLE_ASSERT(c.capacity());
    HashtablezInfoHandle infoz =
  ...
  void GrowSizeIntoSingleGroup(CommonFields& c, Alloc& alloc_ref) {
    Y_ABSL_SWISSTABLE_ASSERT(old_capacity_ < Group::kWidth / 2);
    Y_ABSL_SWISSTABLE_ASSERT(
...
class raw_hash_set {
 ...
 private:
  ...
  template <class T>
  using Insertable = y_absl::disjunction<
      std::is_same<y_absl::remove_cvref_t<reference>, y_absl::remove_cvref_t<T>>,
 ...
 public:
  ...
  Y_ABSL_ATTRIBUTE_NOINLINE explicit raw_hash_set(
    ...
    if (bucket_count > DefaultCapacity()) {
      if (Y_ABSL_PREDICT_FALSE(bucket_count >
  ...
  raw_hash_set(const raw_hash_set& that, const allocator_type& a)
    ...
    if (fits_in_soo(size)) {
      Y_ABSL_SWISSTABLE_ASSERT(size == 1);
      common().set_full_soo();
    ...
    Y_ABSL_SWISSTABLE_ASSERT(!that.is_soo());
    const size_t cap = capacity();
    ...
    IterateOverFullSlots(
        ...
        [&](const ctrl_t* that_ctrl,
          ...
          Y_ABSL_SWISSTABLE_ASSERT(  // We rely that hash is not changed for small
                                   // tables.
  ...
  iterator begin() Y_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    ...
    Y_ABSL_SWISSTABLE_ASSERT(IsFull(*it.control()));
    return it;
  }
  ...
  size_t capacity() const {
    ...
    Y_ABSL_ATTRIBUTE_UNUSED static constexpr size_t kDefaultCapacity =
        DefaultCapacity();
    Y_ABSL_ASSUME(cap >= kDefaultCapacity);
    return cap;
  }
  ...
  std::pair<iterator, bool> insert(
      T&& value Y_ABSL_INTERNAL_ATTRIBUTE_CAPTURED_BY(this))
  ...
  template <class T,
            std::enable_if_t<IsDecomposableAndInsertable<const T&>::value &&
                             ...
                             int> = 0>
  std::pair<iterator, bool> insert(
      const T& value Y_ABSL_INTERNAL_ATTRIBUTE_CAPTURED_BY(this))
      Y_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return emplace(value);
  }
  ...
  template <class T,
            std::enable_if_t<IsDecomposableAndInsertable<T>::value &&
                                 ...
                                 IsLifetimeBoundAssignmentFrom<T>::value,
                             int> = 0>
  iterator insert(const_iterator, T&& value Y_ABSL_INTERNAL_ATTRIBUTE_CAPTURED_BY(
                                      this)) Y_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return insert(std::forward<T>(value)).first;
  }
  ...
  class constructor {
   ...
   public:
    ...
    void operator()(Args&&... args) const {
      Y_ABSL_SWISSTABLE_ASSERT(*slot_);
  ...
  void rehash(size_t n) {
    ...
    if (n == 0) {
      ...
      if (fits_in_soo(size())) {
        ...
        if (infoz().IsSampled()) {
          ...
          Y_ABSL_SWISSTABLE_ASSERT(infoz().IsSampled());
          return;
        }
    ...
    if (n == 0 || m > cap) {
      if (Y_ABSL_PREDICT_FALSE(m > MaxValidCapacity<sizeof(slot_type)>())) {
        HashTableSizeOverflow();
      }
      resize(m);
  ...
  void reserve(size_t n) {
    ...
    if (n > max_size_before_growth) {
      if (Y_ABSL_PREDICT_FALSE(n > max_size())) {
        HashTableSizeOverflow();
      }
  ...
  iterator find(const key_arg<K>& key,
                size_t) Y_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return find(key);
  }
  ...
  const_iterator find(const key_arg<K>& key,
                      size_t) const Y_ABSL_ATTRIBUTE_LIFETIME_BOUND {
    return find(key);
  }
  template <class K = key_type>
 ...
 private:
  ...
  iterator find_soo(const key_arg<K>& key) {
    Y_ABSL_SWISSTABLE_ASSERT(is_soo());
  ...
  iterator find_non_soo(const key_arg<K>& key, size_t hash) {
    ...
    while (true) {
      ...
      seq.next();
      Y_ABSL_SWISSTABLE_ASSERT(seq.index() <= capacity() && "full table!");
    }
  }
  ...
  inline void dealloc() {
    Y_ABSL_SWISSTABLE_ASSERT(capacity() != 0);
    // Unpoison before returning the memory to the allocator.
  ...
  void resize_with_soo_infoz(HashtablezInfoHandle forced_infoz) {
    Y_ABSL_SWISSTABLE_ASSERT(forced_infoz.IsSampled());
  ...
  Y_ABSL_ATTRIBUTE_NOINLINE static void resize_impl(
    ...
    Y_ABSL_SWISSTABLE_ASSERT(IsValidCapacity(new_capacity));
    Y_ABSL_SWISSTABLE_ASSERT(!set->fits_in_soo(new_capacity));
    ...
    Y_ABSL_SWISSTABLE_ASSERT(resize_helper.old_capacity() > 0);
  ...
  void annotate_for_bug_detection_on_move(
      Y_ABSL_ATTRIBUTE_UNUSED raw_hash_set& that) {
 ...
 protected:
  ...
  template <class K>
  void AssertOnFind(Y_ABSL_ATTRIBUTE_UNUSED const K& key) {
  ...
  void AssertNotDebugCapacity() const {
    if (Y_ABSL_PREDICT_TRUE(capacity() <
    ...
    if (SwisstableGenerationsEnabled() &&
        Y_ABSL_PREDICT_FALSE(capacity() >= InvalidCapacity::kMovedFrom)) {
      if (capacity() == InvalidCapacity::kSelfMovedFrom) {
        ...
        Y_ABSL_RAW_LOG(FATAL, "Use of self-move-assigned hash table.");
      }
      Y_ABSL_RAW_LOG(FATAL, "Use of moved-from hash table.");
    }
  }
  ...
  void AssertHashEqConsistent(const K& key) {
    ...
    if (std::is_same<hasher, y_absl::container_internal::StringHash>::value &&
        std::is_same<key_equal, y_absl::container_internal::StringEq>::value) {
      return;
    }
    if (std::is_scalar<key_type>::value &&
        std::is_same<hasher, y_absl::Hash<key_type>>::value &&
    ...
    const auto assert_consistent = [&](const ctrl_t*, slot_type* slot) {
      ...
      Y_ABSL_ATTRIBUTE_UNUSED const bool is_hash_equal =
...
struct HashtableFreeFunctionsAccess {
  ...
  static typename Set::size_type EraseIf(Predicate& pred, Set* c) {
    ...
    if (c->is_soo()) {
      ...
      if (!pred(*it)) {
        Y_ABSL_SWISSTABLE_ASSERT(c->size() == 1 &&
                               "hash table was modified unexpectedly");
        return 0;
      }
      c->destroy(it.slot());
    ...
    Y_ABSL_SWISSTABLE_ASSERT(original_size_for_assert - num_deleted ==
                               c->size() &&
                           "hash table was modified unexpectedly");
    return num_deleted;
  }
...
#undef Y_ABSL_SWISSTABLE_ASSERT
