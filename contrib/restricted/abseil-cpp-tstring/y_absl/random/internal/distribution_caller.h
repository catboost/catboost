...
struct DistributionCaller {
  ...
  using invoke_mock_t = decltype(std::declval<T*>()->InvokeMock(
      std::declval<::y_absl::base_internal::FastTypeIdType>(),
  ...
  static typename DistrT::result_type Impl(std::true_type, URBG* urbg,
    ...
    using ArgTupleT = std::tuple<y_absl::decay_t<Args>...>;
    ...
    if (!urbg->InvokeMock(::y_absl::base_internal::FastTypeId<KeyT>(), &arg_tuple,
                          &result)) {
      auto dist = y_absl::make_from_tuple<DistrT>(arg_tuple);
      result = dist(*urbg);
    }
    return result;
  }
...
#endif  // Y_ABSL_RANDOM_INTERNAL_DISTRIBUTION_CALLER_H_
