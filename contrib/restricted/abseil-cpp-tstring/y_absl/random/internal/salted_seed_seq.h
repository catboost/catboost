...
#ifndef Y_ABSL_RANDOM_INTERNAL_SALTED_SEED_SEQ_H_
#define Y_ABSL_RANDOM_INTERNAL_SALTED_SEED_SEQ_H_
...
class SaltedSeedSeq {
 public:
  ...
  SaltedSeedSeq() : seq_(y_absl::make_unique<SSeq>()) {}

  template <typename Iterator>
  SaltedSeedSeq(Iterator begin, Iterator end)
      : seq_(y_absl::make_unique<SSeq>(begin, end)) {}
 ...
 private:
  ...
  void generate_impl(ContiguousAndUint32Tag, Contiguous begin, Contiguous end,
    ...
    const uint32_t salt = y_absl::random_internal::GetSaltMaterial().value_or(0);
    auto span = y_absl::Span<uint32_t>(&*begin, n);
    MixIntoSeedMaterial(y_absl::MakeConstSpan(&salt, 1), span);
  }
...
template <
    typename SSeq,  //
    typename EnableIf = y_absl::enable_if_t<is_salted_seed_seq<SSeq>::value>>
...
SaltedSeedSeq<typename std::decay<SSeq>::type> MakeSaltedSeedSeq(SSeq&& seq) {
  ...
  y_absl::InlinedVector<result_type, 8> data;
...
#endif  // Y_ABSL_RANDOM_INTERNAL_SALTED_SEED_SEQ_H_
