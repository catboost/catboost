...
#include "y_absl/random/internal/randen_slow.h"
...
#include "y_absl/random/internal/platform.h"
#include "y_absl/random/internal/randen_traits.h"

#if Y_ABSL_HAVE_ATTRIBUTE(always_inline) || \
    (defined(__GNUC__) && !defined(__clang__))
#define Y_ABSL_RANDOM_INTERNAL_ATTRIBUTE_ALWAYS_INLINE \
  __attribute__((always_inline))
#elif defined(_MSC_VER)
// We can achieve something similar to attribute((always_inline)) with MSVC by
// using the __forceinline keyword, however this is not perfect. MSVC is
// much less aggressive about inlining, and even with the __forceinline keyword.
#define Y_ABSL_RANDOM_INTERNAL_ATTRIBUTE_ALWAYS_INLINE __forceinline
#else
#define Y_ABSL_RANDOM_INTERNAL_ATTRIBUTE_ALWAYS_INLINE
#endif
...
inline Y_ABSL_RANDOM_INTERNAL_ATTRIBUTE_ALWAYS_INLINE Vector128
...
inline Y_ABSL_RANDOM_INTERNAL_ATTRIBUTE_ALWAYS_INLINE void Vector128Store(
...
using ::y_absl::random_internal::RandenTraits;

// The improved Feistel block shuffle function for 16 blocks.
inline Y_ABSL_RANDOM_INTERNAL_ATTRIBUTE_ALWAYS_INLINE void BlockShuffle(
    y_absl::uint128* state) {
...
#if 0
  y_absl::uint128 source[RandenTraits::kFeistelBlocks];
  std::memcpy(source, state, sizeof(source));
  for (size_t i = 0; i < RandenTraits::kFeistelBlocks; i++) {
    const y_absl::uint128 v0 = source[shuffle[i]];
    state[i] = v0;
  }
  return;
#endif

  const y_absl::uint128 v0 = state[shuffle[0]];
  const y_absl::uint128 v1 = state[shuffle[1]];
  const y_absl::uint128 v2 = state[shuffle[2]];
  const y_absl::uint128 v3 = state[shuffle[3]];
  const y_absl::uint128 v4 = state[shuffle[4]];
  const y_absl::uint128 v5 = state[shuffle[5]];
  const y_absl::uint128 v6 = state[shuffle[6]];
  const y_absl::uint128 v7 = state[shuffle[7]];
  const y_absl::uint128 w0 = state[shuffle[8]];
  const y_absl::uint128 w1 = state[shuffle[9]];
  const y_absl::uint128 w2 = state[shuffle[10]];
  const y_absl::uint128 w3 = state[shuffle[11]];
  const y_absl::uint128 w4 = state[shuffle[12]];
  const y_absl::uint128 w5 = state[shuffle[13]];
  const y_absl::uint128 w6 = state[shuffle[14]];
  const y_absl::uint128 w7 = state[shuffle[15]];
  state[0] = v0;
  state[1] = v1;
  state[2] = v2;
  state[3] = v3;
...
inline Y_ABSL_RANDOM_INTERNAL_ATTRIBUTE_ALWAYS_INLINE const y_absl::uint128*
FeistelRound(y_absl::uint128* Y_ABSL_RANDOM_INTERNAL_RESTRICT state,
             const y_absl::uint128* Y_ABSL_RANDOM_INTERNAL_RESTRICT keys) {
...
inline Y_ABSL_RANDOM_INTERNAL_ATTRIBUTE_ALWAYS_INLINE void Permute(
    y_absl::uint128* state,
    const y_absl::uint128* Y_ABSL_RANDOM_INTERNAL_RESTRICT keys) {
...
inline Y_ABSL_RANDOM_INTERNAL_ATTRIBUTE_ALWAYS_INLINE void SwapEndian(
    y_absl::uint128* state) {
#ifdef Y_ABSL_IS_BIG_ENDIAN
  for (uint32_t block = 0; block < RandenTraits::kFeistelBlocks; ++block) {
    uint64_t new_lo = y_absl::little_endian::ToHost64(
        static_cast<uint64_t>(state[block] >> 64));
    uint64_t new_hi = y_absl::little_endian::ToHost64(
        static_cast<uint64_t>((state[block] << 64) >> 64));
    state[block] = (static_cast<y_absl::uint128>(new_hi) << 64) | new_lo;
  }
#else
...
void RandenSlow::Absorb(const void* seed_void, void* state_void) {
  auto* state =
      reinterpret_cast<uint64_t * Y_ABSL_RANDOM_INTERNAL_RESTRICT>(state_void);
  const auto* seed =
      reinterpret_cast<const uint64_t * Y_ABSL_RANDOM_INTERNAL_RESTRICT>(
          seed_void);
...
void RandenSlow::Generate(const void* keys_void, void* state_void) {
  static_assert(RandenTraits::kCapacityBytes == sizeof(y_absl::uint128),
                "Capacity mismatch");

  auto* state = reinterpret_cast<y_absl::uint128*>(state_void);
  const auto* keys = reinterpret_cast<const y_absl::uint128*>(keys_void);

  const y_absl::uint128 prev_inner = state[0];
