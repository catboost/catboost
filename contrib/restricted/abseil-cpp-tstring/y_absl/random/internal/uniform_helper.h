...
//
#ifndef Y_ABSL_RANDOM_INTERNAL_UNIFORM_HELPER_H_
#define Y_ABSL_RANDOM_INTERNAL_UNIFORM_HELPER_H_
...
// y_absl::Uniform() will be discarded, and the call will be ill-formed.
// Return-type for y_absl::Uniform() when the return-type is inferred.
template <typename A, typename B>
using uniform_inferred_return_t =
    y_absl::enable_if_t<y_absl::disjunction<is_widening_convertible<A, B>,
...
// are used as implementation-details for y_absl::Uniform().
//
// Conceptually,
...
typename y_absl::enable_if_t<
    y_absl::conjunction<
        IsIntegral<IntType>,
        y_absl::disjunction<std::is_same<Tag, IntervalOpenClosedTag>,
...
typename y_absl::enable_if_t<
    y_absl::disjunction<std::is_same<Tag, IntervalClosedClosedTag>,
...
typename y_absl::enable_if_t<
    y_absl::conjunction<
        IsIntegral<IntType>,
        y_absl::disjunction<std::is_same<Tag, IntervalClosedOpenTag>,
                          std::is_same<Tag, IntervalOpenOpenTag>>>::value,
    IntType>
...
y_absl::enable_if_t<std::is_floating_point<FloatType>::value, bool>
is_uniform_range_valid(FloatType a, FloatType b) {
  return a <= b && std::isfinite(b - a);
}

template <typename IntType>
y_absl::enable_if_t<IsIntegral<IntType>::value, bool>
is_uniform_range_valid(IntType a, IntType b) {
  return a <= b;
}

// UniformDistribution selects either y_absl::uniform_int_distribution
// or y_absl::uniform_real_distribution depending on the NumType parameter.
template <typename NumType>
using UniformDistribution =
    typename std::conditional<IsIntegral<NumType>::value,
                              y_absl::uniform_int_distribution<NumType>,
                              y_absl::uniform_real_distribution<NumType>>::type;
...
#endif  // Y_ABSL_RANDOM_INTERNAL_UNIFORM_HELPER_H_
