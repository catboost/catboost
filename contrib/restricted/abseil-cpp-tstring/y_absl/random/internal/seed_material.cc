...
#define Y_ABSL_RANDOM_USE_NACL_SECURE_RANDOM 1

#elif defined(_WIN32)

#include <windows.h>
#define Y_ABSL_RANDOM_USE_BCRYPT 1
...
#define Y_ABSL_RANDOM_USE_GET_ENTROPY 1
#endif
...
#if defined(Y_ABSL_RANDOM_USE_BCRYPT)
#include <bcrypt.h>
...
// TODO(y_absl-team): Possibly place a cap on how much entropy may be read at a
// time.

#if defined(Y_ABSL_RANDOM_USE_BCRYPT)

// On Windows potentially use the BCRYPT CNG API to read available entropy.
bool ReadSeedMaterialFromOSEntropyImpl(y_absl::Span<uint32_t> values) {
  BCRYPT_ALG_HANDLE hProvider;
  NTSTATUS ret;
  ret = BCryptOpenAlgorithmProvider(&hProvider, BCRYPT_RNG_ALGORITHM,
                                    MS_PRIMITIVE_PROVIDER, 0);
  if (!(BCRYPT_SUCCESS(ret))) {
    Y_ABSL_RAW_LOG(ERROR, "Failed to open crypto provider.");
    return false;
  }
  ret = BCryptGenRandom(
      hProvider,                                             // provider
...
#elif defined(Y_ABSL_RANDOM_USE_NACL_SECURE_RANDOM)
...
bool ReadSeedMaterialFromOSEntropyImpl(y_absl::Span<uint32_t> values) {
  ...
  while (buffer_size > 0) {
    ...
    if (error != 0 || nread > buffer_size) {
      Y_ABSL_RAW_LOG(ERROR, "Failed to read secure_random seed data: %d", error);
      return false;
    }
    output_ptr += nread;
    buffer_size -= nread;
...
#if defined(Y_ABSL_RANDOM_USE_GET_ENTROPY)
...
// false.
bool ReadSeedMaterialFromGetEntropy(y_absl::Span<uint32_t> values) {
  auto buffer = reinterpret_cast<uint8_t*>(values.data());
  size_t buffer_size = sizeof(uint32_t) * values.size();
  while (buffer_size > 0) {
    ...
    Y_ABSL_ANNOTATE_MEMORY_IS_INITIALIZED(buffer, to_read);
    buffer += to_read;
    buffer_size -= to_read;
  }
  return true;
}
#endif  // defined(Y_ABSL_RANDOM_GETENTROPY)

// On *nix, read entropy from /dev/urandom.
bool ReadSeedMaterialFromDevURandom(y_absl::Span<uint32_t> values) {
...
bool ReadSeedMaterialFromOSEntropy(y_absl::Span<uint32_t> values) {
...
void MixIntoSeedMaterial(y_absl::Span<const uint32_t> sequence,
                         y_absl::Span<uint32_t> seed_material) {
