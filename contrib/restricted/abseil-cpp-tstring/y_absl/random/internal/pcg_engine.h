...
#ifndef Y_ABSL_RANDOM_INTERNAL_PCG_ENGINE_H_
#define Y_ABSL_RANDOM_INTERNAL_PCG_ENGINE_H_
...
class pcg_engine {
  static_assert(std::is_same<typename Params::state_type,
                ...
                "Class-template y_absl::pcg_engine must be parameterized by "
 ...
 public:
  ...
  template <class SeedSequence,
            typename = typename y_absl::enable_if_t<
  ...
  friend typename y_absl::enable_if_t<(sizeof(state_type) == 16),
  ...
  template <class CharT, class Traits>
  friend typename y_absl::enable_if_t<(sizeof(state_type) <= 8),
                                    std::basic_ostream<CharT, Traits>&>
  operator<<(
      std::basic_ostream<CharT, Traits>& os,  // NOLINT(runtime/references)
      const pcg_engine& engine) {
...
#endif  // Y_ABSL_RANDOM_INTERNAL_PCG_ENGINE_H_
