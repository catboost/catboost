...
#ifndef Y_ABSL_RANDOM_INTERNAL_IOSTREAM_STATE_SAVER_H_
#define Y_ABSL_RANDOM_INTERNAL_IOSTREAM_STATE_SAVER_H_
...
// the y_absl random distributions.
...
#define Y_ABSL_RANDOM_INTERNAL_IOSTREAM_HEXFLOAT 1
#else
#define Y_ABSL_RANDOM_INTERNAL_IOSTREAM_HEXFLOAT 0
#endif
...
#if Y_ABSL_RANDOM_INTERNAL_IOSTREAM_HEXFLOAT
                                    std::ios_base::fixed |
#endif
...
template <typename T>
typename y_absl::enable_if_t<!std::is_base_of<std::ios_base, T>::value,
                           null_state_saver<T>>
...
// stream_u128_helper allows us to write out either y_absl::uint128 or
...
template <>
struct stream_u128_helper<y_absl::uint128> {
  template <typename IStream>
  inline y_absl::uint128 read(IStream& in) {
    uint64_t h = 0;
    uint64_t l = 0;
    in >> h >> l;
    return y_absl::MakeUint128(h, l);
  }

  template <typename OStream>
  inline void write(y_absl::uint128 val, OStream& out) {
    uint64_t h = y_absl::Uint128High64(val);
    uint64_t l = y_absl::Uint128Low64(val);
    out << h << out.fill() << l;
  }
};
...
#endif  // Y_ABSL_RANDOM_INTERNAL_IOSTREAM_STATE_SAVER_H_
