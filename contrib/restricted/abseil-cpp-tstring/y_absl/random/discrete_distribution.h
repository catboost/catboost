...
#ifndef Y_ABSL_RANDOM_DISCRETE_DISTRIBUTION_H_
...
// y_absl::discrete_distribution takes O(N) time to precompute the probabilities
  ...
  class param_type {
   ...
   private:
    ...
    static_assert(std::is_integral<result_type>::value,
                  "Class-template y_absl::discrete_distribution<> must be "
...
discrete_distribution<IntType>::operator()(
    ...
    const param_type& p) {
  const auto idx = y_absl::uniform_int_distribution<result_type>(0, p.n())(g);
  const auto& q = p.q_[idx];
  const bool selected = y_absl::bernoulli_distribution(q.first)(g);
...
#endif  // Y_ABSL_RANDOM_DISCRETE_DISTRIBUTION_H_
