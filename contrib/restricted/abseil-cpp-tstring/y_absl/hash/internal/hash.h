...
#if defined(__ARM_ACLE) && !defined(__NVCC__)
#include <arm_acle.h>
#endif
...
template <typename H, typename T,
          y_absl::enable_if_t<FitsIn64Bits<T>::value, int> = 0>
H hash_bytes(H hash_state, const T& value) {
  ...
  if (sizeof(T) == 1) {
    ...
    v = y_absl::base_internal::UnalignedLoad16(start);
  } else if (sizeof(T) == 4) {
    v = y_absl::base_internal::UnalignedLoad32(start);
  } else {
    assert(sizeof(T) == 8);
    v = y_absl::base_internal::UnalignedLoad64(start);
  }
...
class Y_ABSL_DLL MixingHashState : public HashStateBase<MixingHashState> {
  ...
  Y_ABSL_CACHELINE_ALIGNED static constexpr uint64_t kStaticRandomData[] = {
 ...
 private:
  ...
  Y_ABSL_ATTRIBUTE_ALWAYS_INLINE static uint64_t CombineSmallContiguousImpl(
  ...
  Y_ABSL_ATTRIBUTE_ALWAYS_INLINE static uint64_t CombineContiguousImpl9to16(
      uint64_t state, const unsigned char* first, size_t len) {
    Y_ABSL_ASSUME(len >= 9);
    Y_ABSL_ASSUME(len <= 16);
  ...
  Y_ABSL_ATTRIBUTE_ALWAYS_INLINE static uint64_t CombineContiguousImpl17to32(
      uint64_t state, const unsigned char* first, size_t len) {
    Y_ABSL_ASSUME(len >= 17);
    Y_ABSL_ASSUME(len <= 32);
...
#endif
    return y_absl::base_internal::UnalignedLoad64(p);
#if defined(__GNUC__) && !defined(__clang__)
#pragma GCC diagnostic pop
  ...
  static uint64_t Read4To8(const unsigned char* p, size_t len) {
    ...
    uint64_t most_significant =
        static_cast<uint64_t>(y_absl::base_internal::UnalignedLoad32(p)) << 32;
    uint64_t least_significant =
        y_absl::base_internal::UnalignedLoad32(p + len - 4);
  ...
  Y_ABSL_ATTRIBUTE_ALWAYS_INLINE static uint64_t Mix(uint64_t lhs, uint64_t rhs) {
  ...
  Y_ABSL_ATTRIBUTE_ALWAYS_INLINE static uint64_t WeakMix(uint64_t n) {
...
#else
    return y_absl::gbswap_64(n * kMul);
#endif
  }
...
inline uint64_t MixingHashState::CombineContiguousImpl(
  ...
  if (Y_ABSL_PREDICT_TRUE(len <= PiecewiseChunkSize())) {
