...
{
    static const char *kwlist[] = {"x","y","new_x","new_y", NULL};
    ...
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OOOO:linear_dddd",(char**)kwlist,&py_x, &py_y, &py_new_x, &py_new_y))
       return NULL;
...
static PyObject* loginterp_method(PyObject*self, PyObject* args, PyObject* kywds)
{
    ...
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OOOO:loginterp_dddd",(char**)kwlist,&py_x, &py_y, &py_new_x, &py_new_y))
       return NULL;
    arr_x = PyArray_FROMANY(py_x, NPY_DOUBLE, 1, 1, NPY_IN_ARRAY);
    if (!arr_x) {
        PyErr_SetString(PyExc_ValueError, "x must be a 1-D array of floats");
...
static PyObject* window_average_method(PyObject*self, PyObject* args, PyObject* kywds)
{
    ...
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OOOOd:loginterp_dddd",(char**)kwlist,&py_x, &py_y, &py_new_x, &py_new_y, &width))
       return NULL;
    arr_x = PyArray_FROMANY(py_x, NPY_DOUBLE, 1, 1, NPY_IN_ARRAY);
    if (!arr_x) {
        PyErr_SetString(PyExc_ValueError, "x must be a 1-D array of floats");
...
#else
PyMODINIT_FUNC init5scipy11interpolate12_interpolate(void)
{
    PyObject* m;
    m = Py_InitModule3("scipy.interpolate._interpolate", interpolate_methods, 
