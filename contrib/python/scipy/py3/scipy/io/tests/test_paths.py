...
class TestPaths:
    ...
    def test_readsav(self):
        path = Path(yc.source_path(__file__)).parent / 'data/scalar_string.sav'
    ...
    def test_netcdf_file(self):
        path = Path(yc.source_path(__file__)).parent / 'data/example_1.nc'
        scipy.io.netcdf_file(path)

    def test_wavfile_read(self):
        path = Path(yc.source_path(__file__)).parent / 'data/test-8000Hz-le-2ch-1byteu.wav'
        scipy.io.wavfile.read(path)

    def test_wavfile_write(self):
        # Read from str path, write to Path
        input_path = Path(yc.source_path(__file__)).parent / 'data/test-8000Hz-le-2ch-1byteu.wav'
