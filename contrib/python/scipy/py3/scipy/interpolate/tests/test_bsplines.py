...
class TestSmoothingSpline:
    ...
    def test_compare_with_GCVSPL(self):
        ...
        >>> x = csvread('x.csv');  # doctest: +SKIP
        >>> y = csvread('y.csv');  # doctest: +SKIP

        Then, in order to access the implementation, we compile gcvspl files in
        Octave:

        >>> mex gcvsplmex.c gcvspl.c  # doctest: +SKIP
        >>> mex spldermex.c gcvspl.c  # doctest: +SKIP
        ...
        >>> c = gcvsplmex( x, y, 2 );  # doctest: +SKIP
        >>> y0 = spldermex( x, c, 2, x, 0 );  # doctest: +SKIP

        If we want to compare the results of the gcvspl code, we can save
        ``y0`` in csv file:

        >>> csvwrite('y0.csv', y0);  # doctest: +SKIP
