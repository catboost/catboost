...
cdef class CParser:
    ...
    cdef size_t stream_cache_len
    cdef size_t stream_cache_pos
    ...
    def __init__(self, stream):
        ...
        if is_readable:
            ...
            self.stream = stream
            yaml_parser_set_input_string(&self.parser, <unsigned char*> PyString_AS_STRING(stream), PyString_GET_SIZE(stream))
    ...
    cdef object _token_to_object(self, yaml_token_t *token):
        ...
        if token.type == YAML_NO_TOKEN:
            ...
            handle = PyUnicode_FromString(<char*> token.data.tag_directive.handle)
            prefix = PyUnicode_FromString(<char*> token.data.tag_directive.prefix)
            ...
            handle = PyUnicode_FromString(<char*> token.data.tag.handle)
            suffix = PyUnicode_FromString(<char*> token.data.tag.suffix)
            if not handle:
                handle = None
        ...
        if event.type == YAML_NO_EVENT:
            ...
            if event.data.document_start.tag_directives.start != NULL:
                ...
                while tag_directive != event.data.document_start.tag_directives.end:
                    handle = PyUnicode_FromString(<char*> tag_directive.handle)
                    prefix = PyUnicode_FromString(<char*> tag_directive.prefix)
                    tags[handle] = prefix
...
cdef int input_handler(void *data, unsigned char *buffer, size_t size, size_t *read) except 0:
    ...
    if size > 0:
        memcpy(<char*> buffer, PyString_AS_STRING(parser.stream_cache)
        ...
        if event_class is StreamStartEvent:
            ...
            if event_object.tags:
                ...
                for handle in event_object.tags:
                    ...
                    tag_directives_end.handle = <unsigned char*> PyString_AS_STRING(handle)
                    ...
                    tag_directives_end.prefix = <unsigned char*> PyString_AS_STRING(prefix)
            ...
            if yaml_alias_event_initialize(event, <unsigned char*> anchor) == 0:
                raise MemoryError
            ...
            if yaml_scalar_event_initialize(event, <unsigned char*> anchor, <unsigned char*> tag, <unsigned char*> value, length,
            ...
            if yaml_sequence_start_event_initialize(event, <unsigned char*> anchor, <unsigned char*> tag,
            ...
            if yaml_mapping_start_event_initialize(event, <unsigned char*> anchor, <unsigned char*> tag,
