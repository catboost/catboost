...
- Model export as generated code tests

  C++ generated code export tests will compile generated C++ code with a C++ compiler, so this compiler must be installed and available from the command line.

  {% list tabs %}

  - Linux

      Install [`g++` compiler](https://gcc.gnu.org/), version 5.4.1 20160904 or later

  - macOS

      Install XCode command line tools (will contain `clang` compiler).
      `g++` alias for `clang` will be used by the tests.

  - Windows

      Install Microsoft Visual Studio 2019 or 2022 or 2026 with C++ x64/x86 build tools.
      Toolset versions supported: 14.28, 14.29, 14.4x, 14.50.
  ...
  1. Open the `catboost/libs/model/model_export/ut/` directory from the local copy of the {{ product }} repository.
  1. Run `python -m pytest` or (if you use `pytest-xdist`) `python -m pytest -n <parallel_worker_count>` or `python -m pytest -n auto` (in the `auto` case the number of parallel workers will be equal to the total count of detected CPU cores).
