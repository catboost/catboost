...
inline ui64 EstimatePerDeviceMemoryUsageForCatFeaturesDataSet(const TDataProvider& dataProvider,
    ...
    return static_cast<ui64>(CompressedSize<ui64>(static_cast<ui32>(dataProvider.GetSampleCount()), maxUniqueValue) * 1.0 * featuresManager.GetCatFeatureIds().size() / NCudaLib::GetCudaManager().GetDeviceCount());
}
...
public:
    ...
    TCompressedCatFeatureDataSetBuilder& Add(ui32 featureId) {
        ...
        const auto& catFeature = dynamic_cast<const TCatFeatureValuesHolder&>(DataProvider.GetFeatureById(dataProviderId));
        const ui64 docCount = catFeature.GetSize();

        auto uncompressedCatFeature = catFeature.ExtractValues();
        TSingleBuffer<ui32> tmp = TSingleBuffer<ui32>::Create(NCudaLib::TSingleMapping(DeviceId, uncompressedCatFeature.size()));
    ...
    void Finish() {
        ...
        MATRIXNET_INFO_LOG << "Build catFeatures compressed dataset "
                           << "for "
                           << DataSet.GetFeatureCount() << " features and " << DataSet.GetDocCount() << " documents" << Endl;

        for (ui32 dev = 0; dev < DevCount; ++dev) {
            MATRIXNET_INFO_LOG << "Memory usage at #" << dev << ": " << sizeof(ui64) * MemoryUsage[dev] * 1.0 / 1024 / 1024 << "MB" << Endl;
        }
        BuildDone = true;
    }
