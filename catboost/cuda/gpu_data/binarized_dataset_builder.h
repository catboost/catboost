    ...
    static TFeaturesSplit Split(const TBinarizedFeaturesManager& grid,
        ...
        if (shuffleFeatures) {
            for (ui32 dev : split.Layout.NonEmptyDevices()) {
                ...
                std::sort(split.FeatureIds.begin() + devSlice.Left, split.FeatureIds.begin() + devSlice.Right, [&](ui32 left, ui32 right) -> bool {
    ...
    static void UpdateFoldOffsets(TDataSet& dataSet) {
        dataSet.Grid.GetMapping().Apply([&](const TSlice& devFeaturesSlice) -> ui64 {
            ...
            for (ui32 i = 0; i < devFeaturesSlice.Size(); ++i) {
                dataSet.HostFeatures[devFeaturesSlice.Left + i].FirstFoldIndex = firstBinOffset;
                firstBinOffset += dataSet.HostFeatures[devFeaturesSlice.Left + i].Folds;
            }
            ...
            for (ui32 feature = 0; feature < devFeatures; feature += featuresPerInt) {
                ...
                for (ui32 i = 0; i < featuresPerInt; ++i) {
                    if (feature + i < devFeatures) {
                        CB_ENSURE(dataSet.HostFeatures[featureIdx].Offset == dataSet.HostFeatures[featureIdx + i].Offset);
                    }
                }
        ...
        auto mapping = dataSet.GetGrid().GetMapping().Transform([&](const TSlice& slice) -> ui64 {
            ...
            for (ui32 f = slice.Left; f < slice.Right; ++f) {
                ...
                for (ui32 fold = 0; fold < feature.Folds; ++fold) {
                    ...
                    dataSet.HostBinFeatures.push_back(binFeature);
                    cursor++;
                }
            }
            return cursor;
...
public:
    ...
    TDataSet Finish() {
        ...
        BuildIsDone = true;
        MATRIXNET_INFO_LOG << "Build dataSet with feature count #" << featureCount << " and doc count #" << DataSet.GetDocumentsMapping().GetObjectsSlice().Size() << Endl;
        MATRIXNET_INFO_LOG << "Features per ui32 " << TGridPolicy::TGridPolicy::FeaturesPerByte() * sizeof(ui32) << Endl;
        const auto& mapping = DataSet.GetCompressedIndex().GetMapping();
        for (auto dev : mapping.NonEmptyDevices()) {
            MATRIXNET_INFO_LOG << "Memory usage for " << DataSet.GetGrid().GetMapping().DeviceSlice(dev).Size() << " features at #" << dev << " " << mapping.MemoryUsageAt(dev) * sizeof(ui32) * 1.0 / 1024 / 1024 << " MB" << Endl;
        }
