...
namespace NCatboostCuda {
    ...
    void TEstimatorsExecutor::ExecEstimators(
        ...
        for (const auto& estimator : estimatorIds) {
            ...
            NCB::TCalculatedFeatureVisitor learnVisitor = std::bind(featureVisitor, learnBinarizedVisitor, std::placeholders::_1, std::placeholders::_2);
            TVector<NCB::TCalculatedFeatureVisitor> testVisitors;
            if (testBinarizedVisitor) {
                testVisitors.push_back(std::bind(featureVisitor, *testBinarizedVisitor, std::placeholders::_1, std::placeholders::_2));
            }
