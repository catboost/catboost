...
inline TMirrorBuffer<ui8> BuildBinarizedTarget(const TBinarizedFeaturesManager& featuresManager,
    ...
    CB_ENSURE(featuresManager.HasTargetBinarization(), "Error: target binarization should be set beforedataSet build");
...
private:
    void WriteFloatFeature(const ui32 feature,
        ...
        if (featureStorage.GetType() == EFeatureValuesType::BinarizedFloat) {
            ...
            auto floatValues = holder.GetValues();

            const auto& borders = FeaturesManager.GetBorders(feature);
            auto bins = BinarizeLine<ui32>(~holder.GetValues(),
                                           holder.GetSize(),
                                           borders);
    ...
    TSingleBuffer<ui64> BuildCompressedBins(const TDataProvider& dataProvider,
        ...
        const TCatFeatureValuesHolder& catFeature = dynamic_cast<const TCatFeatureValuesHolder&>(dataProvider.GetFeatureById(featureId));
...
public:
    ...
    inline TDataSetsHolder<CatFeaturesStoragePtrType> BuildDataSet(const ui32 permutationCount) {
        ...
        {
            yvector<float> joinedTarget = Join(DataProvider.GetTargets(), LinkedTest ? &LinkedTest->GetTargets() : nullptr);
            //ctrs
        ...
        for (ui32 permutationId = 0; permutationId < permutationCount; ++permutationId) {
            dataSetsHolder.PermutationDataSets[permutationId].Reset(new TDataSet<CatFeaturesStoragePtrType>(DataProvider,
                                                                                                            permutationId,
