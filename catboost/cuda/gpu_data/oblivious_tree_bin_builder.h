...
public:
    ...
    TFeatureTensorTracker(TScopedCacheHolder& cacheHolder,
                          ...
                          const TDataSet<CatFeaturesStoragePtrType>& learnSet, const IBinarySplitProvider& binarySplitProvider,
                          const TDataSet<CatFeaturesStoragePtrType>* testSet = nullptr, const IBinarySplitProvider* testBinarySplitProvider = nullptr,
                          ui32 stream = 0)
    ...
    bool AddFeatureTensor(const TFeatureTensor& featureTensor) {
        ...
        for (ui32 catFeature : catFeatures) {
            ...
            if (LinkedTest) {
                TMirrorCatFeatureProvider<CatFeaturesStoragePtrType> broadcastedTestCatFeatures(LinkedTest->GetCatFeatures(),
                                                                                                *CacheHolder);
...
public:
    ...
    const TMirrorBuffer<ui64>& ComputeAndCacheCtrSplit(const TDataSet& dataSet,
        ...
        const TMirrorBuffer<ui64>* compressedSplitTest = LinkedTest ? &CacheTreeCtrSplit(*LinkedTest, split, buildTestBitsFunction) : nullptr;
    ...
    TMirrorBuffer<ui64> CreateSplit(const TMirrorBuffer<float>& ctr,
                                    const float border,
                                    TSlice slice) {
        auto compressedSplit = TMirrorBuffer<ui64>::Create(NCudaLib::TMirrorMapping(CompressedSize<ui64>(static_cast<ui32>(slice.Size()), 2)));
