...
namespace NKernelHost {
    ...
    public:
        ...
        TWriteCompressedSplitKernel(TCFeature feature, ui32 binIdx,
                                    TCudaBufferPtr<const ui32> compressedIndex, ui32 dataSetSize,
                                    TCudaBufferPtr<ui64> compressedBits,
                                    TCudaBufferPtr<const ui32> indices)
            : Feature(feature)
            , BinIdx(binIdx)
        ...
        void Run(const TCudaStream& stream) const {
            NKernel::WriteCompressedSplit(Feature, BinIdx,
                                          CompressedIndex.Get(), DataSetSize, Indices.Get(),
                                          DataSetSize, CompressedBits.Get(),
                                          stream.GetStream());
        }
    };
...
inline void CreateCompressedSplit(const TDataSet& dataSet,
    ...
    LaunchKernels<TKernel>(bits.NonEmptyDevices(), stream, feature, bin,
                           dataSet.GetCompressedIndex(), dataSet.GetDataSetSize(),
                           bits,
                           readIndices);
}
