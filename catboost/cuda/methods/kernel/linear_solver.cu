...
namespace NKernel {
    ...
    __global__ void ExtractMatricesAndTargetsImpl(const float* linearSystem, const int matCount,
                                                  const int rowSize,
                                                  float* matrices, float* targets) {
    ...
    __global__ void SolveForwardImpl(const float* lower, int rowSize, int systemSize, int matCount, float* targets) {
        ...
        for (int row = 0; row < systemSize; ++row) {
            ...
            if (col == 0) {
                solutions[row] = lastCoeff > 0 ? (solutions[row] - sum) / (lastCoeff + 1e-20f) : 0;
            }
    ...
    __global__ void RegularizeImpl(float* lower, int rowSize,
        ...
        __shared__ float countsData[BLOCK_SIZE];
        float* sharedCounts = &countsData[(threadIdx.x / rowSize) * rowSize];
        sharedCounts[col] = lower[col * (col + 1) / 2 + col];
        __syncthreads();
        ...
        for (int row = 0; row < rowSize; ++row) {
            float wj = sharedCounts[row];
            pseudoRank += sharedCounts[row] > 1e-20;
            trace += wj;
        }
        ...
        for (int row = 0; row < rowSize; ++row) {
            ...
            if (col <= row) {
                ...
                val += col < row ? -lambda0 * cellPrior : lambda0 * (1 - cellPrior) + 0.01 * (meanEigenValue) + lambda1;
    ...
    //    line[tid] *= beta[tid];
        ...
        //0.1 * (sum2 - sum * sum / rowSize)  — it's regularization part. we don't use it in score
        //score should be invariant under constant shifts (btw, float aren't)
        if (col == 0) {
            scores[0] = line[tid];
        }
    }
