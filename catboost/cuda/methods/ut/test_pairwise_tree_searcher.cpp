...
SIMPLE_UNIT_TEST_SUITE(TPairwiseHistogramTest) {
    ...
    void inline CalcRefMatrices(EFeaturesGroupingPolicy policy,
        ...
        const TPairwiseTarget& target = subsets.GetPairwiseTarget();
        ...
        for (ui32 dev = 0; dev < GetDeviceCount(); ++dev) {
            ...
            target.PointTarget.DeviceView(dev).Read(gradient);
            weights.resize(gradient.size());

            if (target.PointWeights.GetObjectsSlice().Size()) {
                target.PointWeights.DeviceView(dev).Read(weights);
            }
            ...
            target.PairWeights.DeviceView(dev).Read(pairWeights);
...
//
    void SolveCPU(const TVector<float>& linearSystems,
        ...
        auto computeSolution = [&](int bf) {
            ...
            TVector<double> sqrtMatrix(vecSize);
            ...
            for (ui32 x = 0; x < rowSize; x++) {
                trace += linearSystems[matrixOffset + GetMatrixOffset(x, x)];
                pseudoRank += linearSystems[matrixOffset + GetMatrixOffset(x, x)] > 1e-20;
            }

            float cellPrior = 1.0f / rowSize;
            float meanEigenValue = trace / pseudoRank;

            for (ui32 y = 0; y < vecSize; y++) {
                for (ui32 x = 0; x < vecSize; x++) {
                    ...
                    val += y != x ? -lambda0 * cellPrior : lambda0 * (1 - cellPrior) + 0.01 * (meanEigenValue) + lambda1;
                    sqrtMatrix[y * vecSize + x] = val;
                }
                solution[y] = linearSystems[matrixOffset + vecSize * (vecSize + 1) / 2 + y];
            }
            ...
            for (ui32 i = 0; i < rowSize; ++i) {
                score += ((float) linearSystems[matrixOffset + vecSize * (vecSize + 1) / 2 + i] * solutionFloat[i]);
            }

            {
                ...
                for (ui32 row = 0; row < rowSize; ++row) {
                    score -= 0.5 * betaProj[row] * linearSystems[matrixOffset + vecSize * (vecSize + 1) / 2 + row];
                }
            }
...
//
    void CheckResults(EFeaturesGroupingPolicy policy,
        ...
        const bool nzDiagDer2 =  subsets.GetPairwiseTarget().PointWeights.GetObjectsSlice().Size();
        TVector<float> matrices;
        TVector<float> vectors;
        ...
        UNIT_ASSERT_EQUAL(scoresCpu.size(), scoresGpu.size());
        UNIT_ASSERT_EQUAL(solutionsCpu.size(), solutionsGpu.size());

        for (ui32 i = 0; i < solutionsCpu.size(); ++i) {
            UNIT_ASSERT_DOUBLES_EQUAL_C(solutionsCpu[i], solutionsGpu[i], 1e-5, i << " " << solutionsCpu[i] << " not equal to " << solutionsGpu[i] << " depth " << (1 << depth));
        }

        for (ui32 i = 0; i < scoresCpu.size(); ++i) {
            UNIT_ASSERT_DOUBLES_EQUAL_C(scoresCpu[i], scoresGpu[i], 1e-5, i << " " << scoresCpu[i] << " not equal to " << scoresGpu[i] << " depth " << (1 << depth));
        }
    }

    TPairwiseTarget ComputeWeakTarget(TRandom& random,
                                      NCudaLib::TStripeMapping samplesMapping,
                                      bool useDiagDer2) {
        TPairwiseTarget target;
        target.PointTarget.Reset(samplesMapping);
        ...
        auto pairsMapping = samplesMapping.Transform([&](const TSlice samples) -> ui64 {
            ...
            for (ui32 i = 0; i < estimatedSize; ++i) {
                ...
                if (pair.x != pair.y) {
                    ...
                    const float w = 1.0f;//(random.NextUniformL() % 2) * 1.0 / 2;
                    const float t = (random.NextUniformL() % 4) * w / 4;
            ...
            if (useDiagDer2) {
                for (ui32 i = samples.Left; i < samples.Right; ++i) {
                    const float w = 0.0f;//(random.NextUniformL() % 4) * 1.0 / 4;
...
//                    pointwiseTarget[i] += t;
        ...
        target.PairWeights.Reset(pairsMapping);

        target.Pairs.Write(pairs);
        target.PairWeights.Write(pairWeights);

        target.Docs.Reset(samplesMapping);
        MakeSequence(target.Docs);
        target.PointTarget.Reset(samplesMapping);
        target.PointTarget.Write(pointwiseTarget);

        if (useDiagDer2) {
            target.PointWeights.Reset(samplesMapping);
            target.PointWeights.Write(pointwiseWeights);
        }
        return target;
    }
