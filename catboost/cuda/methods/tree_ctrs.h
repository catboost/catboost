...
private:
    ...
    double MemoryForGridForOneFeatureMb() const {
        return (CtrPerCatFeature * (sizeof(TCFeature) + sizeof(float)) + (sizeof(TCBinFeature) + sizeof(float)) * BinFeatureCountPerCatFeature) * 1.0 / MB;
    }
    ...
    ui32 RequestStream(ui32 featuresToBuild) {
        const ui32 buildStreams = std::min<ui32>(TensorBuilderStreams, featuresToBuild);
        {
            for (ui32 i = static_cast<ui32>(BuilderStreams.size()); i < buildStreams; ++i) {
                BuilderStreams.push_back(buildStreams > 1 ? NCudaLib::GetCudaManager().RequestStream() : NCudaLib::GetCudaManager().DefaultStream());
...
private:
    ...
    TCalcCtrHelper<TMapping>& GetCalcCtrHelper(const TCudaBuffer<ui32, TMapping>& indices,
        ...
        if (CtrHelpers.count(computationStream) == 0) {
            CtrHelpers[computationStream] = MakeHolder<TCalcCtrHelper<TMapping>>(Target, indices, computationStream);
        } else {
...
private:
    THolder<TBinarizedDataSet> CreateBinarizedDataSet() {
        ...
        for (ui32 i = 0; i < ctrs.size(); ++i) {
            dataSet->HostFeatures[i].Folds = TreeCtrDataSet.FeaturesManager.GetCtrBinarization(ctrs[i]).Discretization;
        }
        ...
        NCudaLib::RunPerDeviceSubtasks([&](ui32 device) {
            {
                ...
                AddDataSets(PureTreeCtrDataSets[device], permutationId, true, cachedDataSets);
                ...
                AddDataSets(PureTreeCtrDataSets[device], permutationId, false, withoutCachedIndexDataSets);
                AddDataSets(DataSets[device], permutationId, false, withoutCachedIndexDataSets);
...
private:
    ...
    void ProceedDataSet(ui32 dataSetPermutationKey,
        ...
        if (!dataSet.HasCompressedIndex()) {
            NCudaLib::GetCudaManager().WaitComplete();
            auto guard = NCudaLib::GetCudaManager().GetProfiler().Profile(TStringBuilder() << "Build  #" << dataSet.GetCtrs().size() << " ctrs dataset");
