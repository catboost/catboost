...
private:
    ...
    EBootstrapType BootstrapType = EBootstrapType::Poisson;
    ui64 Seed = 0;
};
...
public:
        ...
        : Config(config)
    {
        ...
        auto mapping = suggestedSeedsMapping.Transform([&](const TSlice& slice) -> ui64 {
            return std::min(maxSeedCount, NHelpers::CeilDivide(slice.Size(), 256) * 256);
        });
...
private:
    ...
    inline void WriteSeedsPointwise(TCudaBuffer<ui64, TMapping>& seeds,
        ...
        for (uint i = 0; i < seeds.GetObjectsSlice().Size(); ++i) {
