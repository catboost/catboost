...
inline void SetBinarizedData(NCompressedPool::TFeatureColumn& column, const yvector<ui64>& data, ui32 bitsPerKey, ui32 len) {
...
private:
    void Split(const TString& pool) {
        ...
        while (input.ReadLine(line)) {
            ...
            CB_ENSURE(words.ysize() == ColumnsDescription.ysize(), TStringBuilder() << "Wrong columns number in pool line: Found " << words.size() << "; Expected " << ColumnsDescription.size());
...
public:
    ...
    TFloatColumnConverter& SetColumn(const yvector<TString>& column) {
        ...
        NPar::ParallelFor(0, (ui32)column.size(), [&](int i) {
            ...
            CB_ENSURE(TryFromString<float>(column[i], val), "Can not parse float factor value " << column[i] << " in column " << i << ". Try correcting column description file.");
            FloatColumn[i] = val;
        });
    ...
    void Write(NCompressedPool::TFeatureColumn& column) {
        ...
        if (BinarizeIt) {
            ...
            MATRIXNET_INFO_LOG << "Compressed feature " << description->GetFeatureId() << " from " << sizeof(float) * FloatColumn.size() / 1024 / 1024 << " to " << sizeof(ui64) * compressedLine.size() / 1024 / 1024 << Endl;
            ...
            *column.MutableFloatColumn()->MutableValues() = ::google::protobuf::RepeatedField<float>(FloatColumn.begin(), FloatColumn.end());
...
public:
    ...
    TCatFeatureColumnConverter Write(NCompressedPool::TFeatureColumn& column) {
        ...
        MATRIXNET_INFO_LOG << "Compressed cat feature " << column.GetFeatureDescription().GetFeatureId() << " from " << sizeof(ui32) * BinarizedData.size() / 1024 / 1024 << " to " << sizeof(ui64) * compressedVector.size() / 1024 / 1024 << Endl;
        MATRIXNET_INFO_LOG << "Bits per key " << bitsPerKey << Endl;
...
public:
    ...
    TCatBoostProtoPoolConverter& SetInputBinarizationFile(const TString& file) {
        Y_ENSURE(BinarizationConfiguration == nullptr, "Error: can't use binarization configuration and existing proto-binarization at the same time");
                ...
                switch (description.Type) {
                    ...
                    case EColumn::Num: {
                        ...
                        if (BinarizationConfiguration) {
                            if (!InputBinarization) {
                                ...
                                converter.BuildBinarized(*GridBuilderFactory, BinarizationConfiguration->DefaultFloatBinarization);
                            } else {
                                ReadMessage(*InputBinarization, floatFeatureBinarization);
                                Y_ENSURE(floatFeatureBinarization.GetFeatureDescription().GetFeatureId() == i, "Error: featureId in index should be equal to featureId in converted pool");
                                yvector<float> borders(floatFeatureBinarization.GetBinarization().borders().begin(), floatFeatureBinarization.GetBinarization().borders().end());
                    ...
                    case EColumn::Categ: {
                        ...
                        if (InputBinarization) {
                            ...
                            Y_ENSURE(binarization.GetFeatureDescription().GetFeatureId() == i, "Error: featureId in index should be equal to featureId in converted pool");
...
private:
    ...
    inline void ConvertColumnAndWrite(ui32 columnId,
        ...
        NPar::ParallelFor(0, strColumn.size(), [&](int i) -> void {
            T val;
            CB_ENSURE(cast(strColumn[i], val), "Column #" + ToString(columnId) + " cannot be parsed. Try correcting column description file. Failed on line " + ToString(i));
