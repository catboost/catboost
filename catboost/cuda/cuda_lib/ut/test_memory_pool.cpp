...
SIMPLE_UNIT_TEST_SUITE(TMemoryPoolTest) {
    ...
    SIMPLE_UNIT_TEST(TestSimpleDefragment) {
...

        {
            ...
            TMemoryCopier<EPtrType::CudaHost, EPtrType::CudaDevice>::CopyMemorySync<char>(~tmp0, block0->Get(), 511 * MB);
        }
        ...
        TPtr block2;
        {
            ...
            TMemoryCopier<EPtrType::Host, EPtrType::CudaDevice>::CopyMemorySync<char>(~tmp, block2->Get(), 255 * MB);
        }

        TPtr block3 = pool.Create(255 * MB);
        TMemoryCopier<EPtrType::CudaDevice, EPtrType::Host>::CopyMemorySync<char>(block2->Get(), ~tmp2, 255 * MB);
    ...
    SIMPLE_UNIT_TEST(TestSimpleDefragment2) {
        ...
        TMemoryCopier<EPtrType::CudaHost, EPtrType::CudaDevice>::CopyMemorySync<char>(~tmp1, block1->Get(), tmp1.size());
        TMemoryCopier<EPtrType::CudaHost, EPtrType::CudaDevice>::CopyMemorySync<char>(~tmp2, block2->Get(), tmp2.size());
        TMemoryCopier<EPtrType::CudaHost, EPtrType::CudaDevice>::CopyMemorySync<char>(~tmp3, block3->Get(), tmp3.size());
        TMemoryCopier<EPtrType::CudaHost, EPtrType::CudaDevice>::CopyMemorySync<char>(~tmp4, block4->Get(), tmp4.size());

        block2.Reset(nullptr);

        TPtr block5 = pool.Create(tmp5.size());

        TMemoryCopier<EPtrType::CudaHost, EPtrType::CudaDevice>::CopyMemorySync<char>(~tmp5, block5->Get(), tmp5.size());

        {
            TVector<char> tmp(tmp1.size());
            TMemoryCopier<EPtrType::CudaDevice, EPtrType::CudaHost>::CopyMemorySync<char>(block1->Get(), ~tmp, tmp1.size());
...

        {
            ...
            TMemoryCopier<EPtrType::CudaDevice, EPtrType::CudaHost>::CopyMemorySync<char>(block3->Get(), ~tmp, tmp3.size());
...

        {
            ...
            TMemoryCopier<EPtrType::CudaDevice, EPtrType::CudaHost>::CopyMemorySync<char>(block4->Get(), ~tmp, tmp4.size());
...

        {
            ...
            TMemoryCopier<EPtrType::CudaDevice, EPtrType::CudaHost>::CopyMemorySync<char>(block5->Get(), ~tmp, tmp5.size());
