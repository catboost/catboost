...
static NCB::TPoolQuantizationSchema LoadInMatrixnetFormat(IInputStream* const input) {
    ...
    schema.TrueFeatureIndices.reserve(remapping.size());
    for (const auto& kv : remapping) {
        schema.TrueFeatureIndices.push_back(kv.first);
    }

    Sort(schema.TrueFeatureIndices);
    ...
    for (size_t i = 0; i < remapping.size(); ++i) {
        ...
        schema.Borders[i] = borders[remapping[schema.TrueFeatureIndices[i]]];
...
void SaveInMatrixnetFormat(
    ...
    for (size_t i = 0; i < schema.TrueFeatureIndices.size(); ++i) {
        for (size_t j = 0; j < schema.Borders[i].size(); ++j) {
            (*output)
                << schema.TrueFeatureIndices[i] << '\t'
                << schema.Borders[i][j];
...
NCB::TPoolQuantizationSchema NCB::QuantizationSchemaFromProto(
    ...
    schema.TrueFeatureIndices.reserve(proto.GetFeatureIndexToSchema().size());
    ...
    schema.Borders.resize(schema.TrueFeatureIndices.size());
    schema.NanModes.resize(schema.TrueFeatureIndices.size());
    for (size_t i = 0; i < schema.TrueFeatureIndices.size(); ++i) {
        const auto& featureSchema = proto.GetFeatureIndexToSchema().at(schema.TrueFeatureIndices[i]);
...
NCB::NIdl::TPoolQuantizationSchema NCB::QuantizationSchemaToProto(
    ...
    for (size_t i = 0; i < schema.TrueFeatureIndices.size(); ++i) {
        ...
        proto.MutableFeatureIndexToSchema()->insert({
            static_cast<ui32>(schema.TrueFeatureIndices[i]),
