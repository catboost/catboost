...
Y_UNIT_TEST(PrecisionAtKTest) {
    {
        ...
        const auto metric = MakePrecisionAtKMetric(ELossFunction::PrecisionAt, /*params=*/{}, 2);
...
Y_UNIT_TEST(RecallAtKTest) {
    {
        ...
        NPar::TLocalExecutor executor;
        const auto metric = MakeRecallAtKMetric(ELossFunction::RecallAt, /*params=*/{}, 2);
        TMetricHolder score = metric->Eval(approx, target, weight, queries, 0, 1, executor);
...
Y_UNIT_TEST(MAPAtKTest) {
    {
        ...
        NPar::TLocalExecutor executor;
        const auto metric = MakeMAPKMetric(ELossFunction::MAP, /*params=*/{});
        TMetricHolder score = metric->Eval(approx, target, weight, queries, 0, 1, executor);
    ...
    }
    {
        ...
        NPar::TLocalExecutor executor;
        const auto metric = MakeMAPKMetric(ELossFunction::MAP, /*params=*/{}, 2);
        TMetricHolder score = metric->Eval(approx, target, weight, queries, 0, 1, executor);
