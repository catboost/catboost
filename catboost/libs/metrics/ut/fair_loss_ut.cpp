...
Y_UNIT_TEST(FairLossTest) {
    {
        ...
        const auto metric = std::move(CreateMetric(ELossFunction::FairLoss, /*params=*/{{"smoothness", "1.0"}}, /*approxDimension=*/1)[0]);
    ...
    }
    {
        ...
        NPar::TLocalExecutor executor;
        const auto metric = std::move(CreateMetric(ELossFunction::FairLoss, /*params=*/{{"smoothness", "10.0"}}, /*approxDimension=*/1)[0]);
        metric->UseWeights = true;
        TMetricHolder score = metric->Eval(approx, target, weight, queries, 0, target.size(), executor);
