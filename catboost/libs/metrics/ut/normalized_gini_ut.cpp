...
Y_UNIT_TEST(BinClassTest) {
    {
        ...
        const auto metric = MakeBinClassNormalizedGiniMetric(ELossFunction::NormalizedGini, /*params=*/{});
...
Y_UNIT_TEST(MultiClassTest) {
    {
        ...
        NPar::TLocalExecutor executor;
        const auto metric = MakeMultiClassNormalizedGiniMetric(ELossFunction::NormalizedGini, /*params=*/{}, 1);
        TMetricHolder score = metric->Eval(approx, target, weight, queries, 0, target.size(), executor);
