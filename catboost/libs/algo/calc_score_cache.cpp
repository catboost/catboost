...
void TCalcScoreFold::Create(const TFold& fold) {
...
void TCalcScoreFold::Sample(const TFold& fold, const TVector<TIndexType>& indices, float sampleRate, TRestorableFastRng64* rand, NPar::TLocalExecutor* localExecutor) {
    ...
    TVectorSlicing dstBlocks;
    SetSampledControl(indices.ysize(), sampleRate, rand);
    dstBlocks.CreateByControl(blockParams, Control, localExecutor);
...
void TCalcScoreFold::SetSampledControl(int docCount, float sampleRate, TRestorableFastRng64* rand) {
    ...
    for (int docIdx = 0; docIdx < docCount; ++docIdx) {
        Control[docIdx] = rand->GenRandReal1() < sampleRate;
    }
}
