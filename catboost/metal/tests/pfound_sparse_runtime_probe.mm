...
struct Params {
    ...
    float L2,NonDiag,Temperature,Subsample;uint32_t Compact,Sparse;
};
extern "C" int cbm_pfound_sparse_runtime_probe(const Params* input,uint64_t budget,
    ...
    @autoreleasepool {try {
        ...
        if(!p.Passes || p.Passes>10 || p.Iterations>100 || p.LeafMethod>1 || p.ScoreMethod>1 || p.GroupSampling>1 || p.Compact>1 || p.Sparse>1)
            throw std::runtime_error("Invalid PFound probe iterations/method");
        auto device=MTLCreateSystemDefaultDevice();
        CBMPFoundPairRuntime runtime(device,p.Rows,p.Groups,groupOffsets,target,objectWeights,p.Permutations,.85f,p.Leaves,p.Candidates,8,budget,p.Compact,p.Sparse);
        ...
        if(p.Compact && p.Sparse) {
            uint64_t sampledRows=0;
            for(uint32_t q=0;q<p.Groups;++q)sampledRows+=std::min(groupOffsets[q+1]-groupOffsets[q],cap);
            pairCapacity=std::min<uint64_t>(pairCapacity,sampledRows*p.Permutations);
        }
        ...
        auto copy=[&](uint32_t index){const auto s=runtime.SampledShape();shapes[5*index]=s.Rows;shapes[5*index+1]=s.Groups;shapes[5*index+2]=s.Pairs;shapes[5*index+3]=runtime.PairCount();shapes[5*index+4]=runtime.GeneratedStorageCount();
            std::memcpy(documents+uint64_t(index)*p.Rows,runtime.SampledDocuments().contents,4ull*s.Rows);
            std::memcpy(pairs+uint64_t(index)*2*pairCapacity,runtime.GeneratedPairs().contents,8ull*runtime.GeneratedStorageCount());
            std::memcpy(edges+uint64_t(index)*4*pairCapacity,runtime.GeneratedEdges().contents,16ull*runtime.GeneratedStorageCount());
            std::memcpy(matrices+uint64_t(index)*pairCapacity,runtime.GeneratedMatrix().contents,4ull*runtime.GeneratedStorageCount());};
