"""Native categorical YetiRank multi-cursor stochastic training and recovery."""
import json
import os
...
pytestmark = pytest.mark.skipif(os.environ.get('CATBOOST_NATIVE_METAL_QUERY_TESTS') != '1',
                               reason='requires rebuilt native YetiRank P4 adapter')
...
def inputs(kind, sampler, leaf_iterations, history='Group'):
    x,y,groups,po,options,offsets = problem('YetiRank','Newton',kind,history=history)
    options.update(has_time=False,permutation_count=4,bootstrap_type=sampler,
                   leaf_estimation_iterations=leaf_iterations,random_strength=.7)
    if sampler in ('Bernoulli','Poisson','MVS'): options['subsample'] = .7
...
def test_native_yeti_p4_ctrs_restore_oracle_rng_cursors_metrics_and_readers(tmp_path, kind, sampler, leaf_iterations, prequantized):
    x,y,_,po,options,_ = inputs(kind,sampler,leaf_iterations,'Sample' if prequantized else 'Group')
    pool = Pool(x,y,**po)
    ...
    saved = options | dict(save_snapshot=True,snapshot_interval=0,snapshot_file='yeti-p4.snapshot',
    ...
    for fmt in ('cbm','json'):
        path = tmp_path/('yeti-p4.'+fmt); direct.save_model(path,format=fmt)
...
@pytest.mark.parametrize('sampler', SAMPLERS)
@pytest.mark.parametrize('leaf_iterations', [1,3])
def test_native_yeti_p4_initial_model_baseline_and_snapshot_resume(tmp_path, sampler, leaf_iterations):
    x,y,_,po,options,_ = inputs('FloatTargetMeanValue',sampler,leaf_iterations)
    pool = Pool(x,y,**po)
...
def test_native_p4_forest_matches_independent_ctr_histories_and_resident_oracle(kind, sampler, leaf_iterations, history):
    ...
    x,y,_,po,options,_ = inputs(kind,sampler,leaf_iterations,history)
    ...
    args = {name:options[name] for name in ('iterations','depth','learning_rate','l2_leaf_reg',
        'leaf_estimation_iterations','bootstrap_type','random_seed','random_strength','score_function')}
    if sampler == 'Bernoulli': args['subsample'] = .7
    with _yeti.TrainingSession(matrices[0],targets,np.zeros(len(borders),np.uint32),np.arange(len(borders),dtype=np.uint32),
            group_offsets=offsets,sample_weight=weights,permutations=7,decay=.85,dataset_permutations=4,**args) as oracle:
