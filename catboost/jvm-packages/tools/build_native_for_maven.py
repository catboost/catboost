...
def _re_match_line_in_file(file_name: str, regexp: str) -> bool:
...
def _get_package_resources_dir(base_dir: str) -> str:
    return os.path.join(base_dir, 'src', 'main', 'resources')


def _get_native_lib_dir(root_dir: str, package_arcadia_path: str) -> str:
    return os.path.join(root_dir, package_arcadia_path, 'src', 'native_impl')


def _build_native_platform_to_jvm_platform(build_native_platform: str) -> str:
...
def _get_platform_resources_dir(parsed_args) -> str:
...
def _is_dll_java(project_path: str) -> bool:
...
def _get_ya_make_platform_from_build_native_platform(target_platform: str) -> str:
...
def build_shared_lib_with_ya(parsed_args, top_src_root_dir: str, package_src_sub_path: str, extra_ya_make_args):
...
def postprocess_after_ya(native_lib_build_dir: str, lib_name: str, resources_dir, base_dir: str):
...
def postprocess_after_cmake(parsed_args, top_src_root_dir: str, resources_dir: str):
