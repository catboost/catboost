...
train<-read.table(file="parsed_train", head=F, sep="\t")
valid<-read.table(file="parsed_test", head=F, sep="\t")
train[,1] <- as.numeric(train[,1])
valid[,1] <- as.numeric(valid[,1])
y <- "V1"
x <- setdiff(names(train), c(y))

port = sample(0:65536, 1)
localH2O = h2o.init(nthreads=-1, max_mem_size='20G', port=port)
...
get_best_cv <- function(model, nfolds = 5){
    ...
    for (i in 1:nfolds){
        ...
        scoring_history <- data.frame(cv_model@model$scoring_history)[,c('number_of_trees', 'validation_rmse')]
        scoring_history_stats <- rbind(scoring_history_stats, scoring_history)
    }
    scoring_history_avg <- dcast(scoring_history_stats, number_of_trees ~ ., mean, value.var = "validation_rmse")
    colnames(scoring_history_avg) <- c('n_trees', 'rmse')
    min_rmse <- min(scoring_history_avg$rmse)
    best_n_trees <- scoring_history_avg[scoring_history_avg$rmse==min_rmse,]$n_trees
    return(list(rmse_cv = min_rmse,
...
get_best_model <- function(gbm_cv_perf){
    ...
    for (i in 1:length(gbm_cv_perf@model_ids)){
        ...
        if (model_cv$rmse_cv < best_rmse){
            best_rmse <- model_cv$rmse_cv
            best_cv <- model_cv
...
validate_model <- function(x, y, train, validate, params, n_trees, seed){
    ...
    rmse_val <- model_metrics@metrics$RMSE
    return(rmse_val)
}
...
for (seed in seeds) {
    set.seed(seed)
    default_rmse_val <- validate_model(x, y, h2_train, h2_valid, best_params, best_cv$n_trees, seed)
    seeds_default <- c(seeds_default, default_rmse_val)
}
...
rmse_val_mean <- seeds_default["mean",]
...
result <- rbind(result,
                ...
                rmse_cv = best_cv$rmse_cv,
                rmse_val = rmse_val_mean)
colnames(result) <- "Default"

write.table(result, file='result_default.tsv', quote=FALSE, sep='\t')
write.table(seeds_default, file='result_default_seeds.tsv', quote=FALSE, sep='\t')
...
for (seed in seeds) {
    set.seed(seed)
    tuned_rmse_val <- validate_model(x, y, h2_train, h2_valid, best_model$best_params, best_model$best_cv$n_trees, seed)
    seeds_tuned <- c(seeds_tuned, tuned_rmse_val)
}
...
rmse_val_mean <- seeds_tuned["mean",]
...
result <- rbind(result,
                ...
                rmse_cv = best_model$best_cv$rmse_cv,
                rmse_val = rmse_val_mean)
colnames(result) <- "Tuned"

write.table(result, file='result_tuned.tsv', quote=FALSE, sep='\t')
write.table(seeds_tuned, file='result_tuned_seeds.tsv', quote=FALSE, sep='\t')
cat("Tuned finished\n")
