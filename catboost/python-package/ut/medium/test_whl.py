...
def test_wheel():
    ...
    for module_name, arc_path, dst_subdir in (
        ...
        src = yatest.common.binary_path(os.path.join(arc_path, py_trait.so_name(module_name)))
        so_modules[module_name] = (src, dst_subdir)

    wheel_name = 'catboost.whl'

    make_wheel(wheel_name, 'catboost', '0.0.0', yatest.common.source_path('.'), so_modules, should_build_widget=False)
