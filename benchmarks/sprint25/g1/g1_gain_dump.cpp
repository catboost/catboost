// g1_gain_dump.cpp — Sprint 25 DEC-026 G1 ε-calibration instrument
//
// PURPOSE
//   Derived from catboost/mlx/tests/bench_boosting.cpp.  Adds two
//   instrumentation surfaces on top of the production GBDT pipeline:
//
//   1. --kernel {t1|t2_path5}  — selects which histogram kernel runs.
//        t1        : production v5 (NCatboostMlx::DispatchHistogramT2 all-features-SIMD),
//                    bit-identical to the T1 reference at the 18 DEC-008 configs.
//        t2_path5  : G1 Path 5 reconstruction (G1Dispatch::DispatchHistogramT2Path5 —
//                    feat-0 bin-range scan over sortedDocs, int-atomic fixed-point
//                    features 1-3).  Produces Value B at config #8 deterministically.
//
//   2. --emit-gain-trace PATH --config-id N --run-id N
//        Enables per-(iter, depth_level) dump of the full candidate-GAIN vector
//        (one GAIN per bin-feature per depth level per iter).  At each step we
//        record the argmax winner and keep `--gain-topk` (default 3) runners-up
//        plus the min non-winner gap (Tail 2).  Output is a gzipped CSV at
//        PATH with schema (config_id, run_id, kernel, iter, node_id, feat,
//        bin, gain, is_winner, rank).
//
// This binary does NOT ship.  It is G1-measurement scaffolding under
// `benchmarks/sprint25/g1/` per DEC-026-G1.  Production kernel_sources.h
// is not modified.
//
// USAGE
...
// G1-only: dump kernel source + Path 5 dispatch.  These headers live under
// benchmarks/sprint25/g1/ and are not part of production.
#include "kernels/g1_kernels.h"
#include "methods/g1_path5_dispatch.h"
...
struct TBenchConfig {
    ...
    // --- G1 additions ---
    std::string KernelChoice = "t1";   // "t1" or "t2_path5"
    std::string GainTracePath;         // if non-empty, emit gzipped CSV of per-candidate GAINs
    ui32 ConfigId = 0;                 // written into every trace row
    ui32 RunId    = 0;                 // written into every trace row
    ui32 GainTopK = 3;                 // number of runners-up to keep per (iter, depth)
};
...
TBenchConfig ParseArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        // --- G1 flags ---
        else if (strcmp(argv[i], "--kernel")          == 0 && i+1 < argc) cfg.KernelChoice    = argv[++i];
        else if (strcmp(argv[i], "--emit-gain-trace") == 0 && i+1 < argc) cfg.GainTracePath   = argv[++i];
        else if (strcmp(argv[i], "--config-id")       == 0 && i+1 < argc) cfg.ConfigId        = std::atoi(argv[++i]);
        else if (strcmp(argv[i], "--run-id")          == 0 && i+1 < argc) cfg.RunId           = std::atoi(argv[++i]);
        else if (strcmp(argv[i], "--gain-topk")       == 0 && i+1 < argc) cfg.GainTopK        = std::atoi(argv[++i]);
        else {
            fprintf(stderr, "Unknown argument: %s\n", argv[i]);
            fprintf(stderr,
                "Usage: g1_gain_dump [--rows N] [--features N] [--classes N]\n"
                "                    [--depth D] [--iters N] [--bins B]\n"
                "                    [--lr F] [--l2 F] [--seed N] [--onehot N]\n"
                "                    [--stage-profile] [--per-kernel-profile]\n"
                "                    [--kernel {t1|t2_path5}] [--emit-gain-trace PATH]\n"
                "                    [--config-id N] [--run-id N] [--gain-topk K]\n");
            exit(1);
        }
    }
...
static mx::array DispatchHistogramT2Bench(
    ...
    // G1: select between production v5 (T1-style all-features SIMD) and Path 5
    // reconstruction (feat-0 bin-range scan + int-atomic fixed-point features 1-3).
    // The kernel-choice string is set once in main() and threaded through a
    // global here; RunIteration does not need to know which kernel is active.
    extern std::string G_KERNEL_CHOICE;
    if (G_KERNEL_CHOICE == "t2_path5") {
        return G1Dispatch::DispatchHistogramT2Path5(
...
// ============================================================================
// G1: global kernel-choice + per-(config, run, iter, depth) GAIN trace state.
//
// Kept as translation-unit globals rather than threaded through every
// function parameter — the bench harness was not designed for config-scoped
// state and G1 is temporary instrumentation.
// ============================================================================

std::string G_KERNEL_CHOICE = "t1";  // set by main()

struct GainTraceRow {
    uint32_t config_id;
    uint32_t run_id;
    uint32_t iter;
    uint32_t depth_level;   // 0..maxDepth-1
    uint32_t feat;
    uint32_t bin;
    float    gain;
    uint8_t  is_winner;     // 1 = argmax at this (iter, depth)
    uint8_t  rank;          // 0 = winner, 1..K = runner-up rank, 255 = min-non-winner sentinel
};

struct GainTraceContext {
    bool enabled = false;
    std::string path;
    uint32_t config_id = 0;
    uint32_t run_id    = 0;
    uint32_t top_k     = 3;
    uint32_t current_iter = 0;      // set by RunIteration
    const char* kernel_tag = "t1";  // pointer to static string; const-lifetime
    std::vector<GainTraceRow> rows; // appended in scoring; flushed at end of run
};

GainTraceContext G_TRACE;  // translation-unit singleton; initialised in main()

// ============================================================================
// ScoreSplitsDumpGPU — per-candidate GAIN dump via kScoreSplitsDumpSource.
//
// Runs the same suffix-sum kernel as FindBestSplitGPU (to produce the
// right-side-sum histogram), then invokes the G1 dump kernel to emit the
// full per-bin GAIN tensor without the argmax reduction.
//
// Returns the raw per-candidate GAIN array (length totalBinFeatures) so the
// caller can decimate to winner + top-K + min-non-winner.  This is a
// simplified FindBestSplitGPU clone — we do not reuse that function because
// it is hard-coded to the argmax-reduced scoring kernel.
// ============================================================================
struct TGainDump {
    std::vector<float>    Gain;   // [totalBinFeatures]
    std::vector<uint32_t> Feat;   // [totalBinFeatures]
    std::vector<uint32_t> Bin;    // [totalBinFeatures]
};

TGainDump ScoreSplitsDumpGPU(
    const mx::array& histogram,
    ...
    TGainDump out;
    const ui32 numFeatures = static_cast<ui32>(features.size());
    if (totalBinFeatures == 0 || numPartitions == 0) return out;
    ...
    // Phase A: suffix-sum (identical to FindBestSplitGPU path).
    auto suffixKernel = mx::fast::metal_kernel(
        ...
        {"histogram_out"},
        NCatboostMlx::KernelSources::kSuffixSumSource,
        NCatboostMlx::KernelSources::kScoreHeader,
        /*ensure_row_contiguous=*/true,
    ...
    // Phase B: per-candidate GAIN via kScoreSplitsDumpSource.
    const ui32 numBlocks = (totalBinFeatures + 255) / 256;
    auto dumpKernel = mx::fast::metal_kernel(
        "score_splits_dump_g1",
        ...
        {"gainOut", "featOut", "binOut"},
        G1Kernels::kScoreSplitsDumpSource,
        NCatboostMlx::KernelSources::kScoreHeader,
        /*ensure_row_contiguous=*/true,
        /*atomic_outputs=*/false
    );

    auto dumpGrid = std::make_tuple(static_cast<int>(256 * numBlocks), 1, 1);
    auto dumpTG   = std::make_tuple(256, 1, 1);

    auto dumpResult = dumpKernel(
        ...
        {{static_cast<int>(totalBinFeatures)},
         {static_cast<int>(totalBinFeatures)},
         {static_cast<int>(totalBinFeatures)}},
    ...
    mx::eval({dumpResult[0], dumpResult[1], dumpResult[2]});

    out.Gain.assign(dumpResult[0].data<float>(),
                    dumpResult[0].data<float>() + totalBinFeatures);
    out.Feat.assign(dumpResult[1].data<uint32_t>(),
                    dumpResult[1].data<uint32_t>() + totalBinFeatures);
    out.Bin.assign (dumpResult[2].data<uint32_t>(),
                    dumpResult[2].data<uint32_t>() + totalBinFeatures);
    return out;
}

// Decimate a full GAIN dump to winner + top-K runners-up + min-non-winner.
// Records rows into G_TRACE.rows with the config/run/iter/depth_level metadata.
static void AppendDumpToTrace(
    const TGainDump& dump,
    uint32_t iter,
    uint32_t depth_level,
    uint32_t top_k
) {
    const size_t N = dump.Gain.size();
    if (N == 0) return;

    // Rank candidates by GAIN descending.  Partial sort — we only need top (K+1)
    // and the strict min-non-winner.
    std::vector<size_t> idx(N);
    std::iota(idx.begin(), idx.end(), size_t(0));

    const size_t keep = std::min<size_t>(top_k + 1u, N);
    std::partial_sort(idx.begin(), idx.begin() + keep, idx.end(),
                      [&](size_t a, size_t b) {
                          return dump.Gain[a] > dump.Gain[b];
                      });

    // Winner + top-K runners-up.
    for (size_t r = 0; r < keep; ++r) {
        GainTraceRow row;
        row.config_id   = G_TRACE.config_id;
        row.run_id      = G_TRACE.run_id;
        row.iter        = iter;
        row.depth_level = depth_level;
        row.feat        = dump.Feat[idx[r]];
        row.bin         = dump.Bin [idx[r]];
        row.gain        = dump.Gain[idx[r]];
        row.is_winner   = (r == 0) ? 1u : 0u;
        row.rank        = static_cast<uint8_t>(r);
        G_TRACE.rows.push_back(row);
    }

    // Min non-winner candidate (for Tail 2 legitimate-gap lower bound).
    // Find argmin over non-winner candidates of GAIN_candidate (so
    // gap = GAIN_winner - min_GAIN among candidates that were actually
    // "considered" in production, i.e. with finite / nonzero gain).  The
    // dump kernel writes 0.0 for invalid candidates (no partition contributes);
    // we exclude those from the min.
    if (N > 1) {
        const size_t winner = idx[0];
        float minGain = std::numeric_limits<float>::infinity();
        size_t minIdx = SIZE_MAX;
        for (size_t i = 0; i < N; ++i) {
            if (i == winner) continue;
            // Skip invalid / zero-gain candidates so Tail 2 reflects legitimate
            // GAIN gaps among viable splits only.  GAIN == 0 typically means
            // every partition had weightLeft<1e-15 or weightRight<1e-15 (the
            // production kernel would set myGain = -INFINITY here).
            if (!(dump.Gain[i] > 0.0f)) continue;
            if (dump.Gain[i] < minGain) {
                minGain = dump.Gain[i];
                minIdx  = i;
            }
        }
        if (minIdx != SIZE_MAX) {
            ...
            row.iter        = iter;
            row.depth_level = depth_level;
            row.feat        = dump.Feat[minIdx];
            row.bin         = dump.Bin [minIdx];
            row.gain        = dump.Gain[minIdx];
            row.is_winner   = 0u;
            row.rank        = 255u;  // sentinel: min-non-winner
            G_TRACE.rows.push_back(row);
        }
    }
}
...
long long RunIteration(
    ...
    for (ui32 depth = 0; depth < maxDepth; ++depth) {
        ...
        if (pkOut) {
            ...
            // G1: per-candidate GAIN dump (before argmax-reduced FindBestSplitGPU).
            // Runs only when --emit-gain-trace is active; dump kernel shares the
            // suffix-sum output with the argmax-reduced scoring path.
            if (G_TRACE.enabled) {
                auto dump = ScoreSplitsDumpGPU(
                    histogram, gradSumsGPU, hessSumsGPU,
                    features, numActiveParts, totalBinFeatures, numStats,
                    l2RegLambda, 1 /*approxDim*/
                );
                AppendDumpToTrace(dump, G_TRACE.current_iter, depth, G_TRACE.top_k);
            }
...
int main(int argc, char** argv) {
    ...
    // G1: apply kernel-choice + trace config.
    if (cfg.KernelChoice != "t1" && cfg.KernelChoice != "t2_path5") {
        fprintf(stderr, "FATAL: --kernel must be 't1' or 't2_path5' (got '%s')\n",
                cfg.KernelChoice.c_str());
        return 1;
    }
    G_KERNEL_CHOICE = cfg.KernelChoice;
    G_TRACE.enabled   = !cfg.GainTracePath.empty();
    G_TRACE.path      = cfg.GainTracePath;
    G_TRACE.config_id = cfg.ConfigId;
    G_TRACE.run_id    = cfg.RunId;
    G_TRACE.top_k     = cfg.GainTopK;
    G_TRACE.kernel_tag= (cfg.KernelChoice == "t2_path5") ? "t2_path5" : "t1";
    if (G_TRACE.enabled) {
        // Conservative reservation: 18 configs × 5 runs × 50 iters × 6 depth × (K+2) rows
        // At K=3 we get ~27000 rows/run, well under default vector growth threshold.
        G_TRACE.rows.reserve(size_t(cfg.NumIters) * size_t(cfg.MaxDepth) * size_t(cfg.GainTopK + 2));
    }

    printf("g1_gain_dump — Sprint 25 DEC-026 G1 ε calibration\n");
    printf("================================================================\n");
    printf("kernel=%s  config_id=%u  run_id=%u  gain_topk=%u  trace=%s\n",
           cfg.KernelChoice.c_str(), cfg.ConfigId, cfg.RunId, cfg.GainTopK,
           G_TRACE.enabled ? cfg.GainTracePath.c_str() : "(disabled)");
    printf("----------------------------------------------------------------\n");
    ...
    auto runBench = [&](TPerKernelTimings& timingsOut) -> float {
        ...
        for (ui32 iter = 0; iter < cfg.NumIters; ++iter) {
            ...
            // G1: expose the current iter index to AppendDumpToTrace, which is
            // called inside RunIteration's depth loop.
            G_TRACE.current_iter = iter;
    ...
    // G1: flush the GAIN trace to CSV (append mode — sweep driver creates the
    // header once per output file).  Columns:
    //   config_id, run_id, kernel, iter, depth_level, feat, bin, gain,
    //   is_winner, rank
    if (G_TRACE.enabled && !G_TRACE.rows.empty()) {
        const bool exists = std::ifstream(G_TRACE.path).good();
        std::ofstream ofs(G_TRACE.path, std::ios::out | std::ios::app);
        if (!ofs) {
            fprintf(stderr, "FATAL: could not open gain-trace path for append: %s\n",
                    G_TRACE.path.c_str());
            return 2;
        }
        if (!exists) {
            ofs << "config_id,run_id,kernel,iter,depth_level,feat,bin,gain,is_winner,rank\n";
        }
        // Write with enough precision that Tail-1 differences at the 1e-7 relative
        // scale survive the CSV round-trip.  float32 has ~7 decimal digits of
        // precision; 9 digits is the round-trip-exact convention (printf %.9g).
        ofs.precision(9);
        for (const auto& r : G_TRACE.rows) {
            ofs << r.config_id << ',' << r.run_id << ',' << G_TRACE.kernel_tag << ','
                << r.iter << ',' << r.depth_level << ','
                << r.feat << ',' << r.bin << ','
                << r.gain << ','
                << unsigned(r.is_winner) << ',' << unsigned(r.rank) << '\n';
        }
        ofs.close();
        printf("[G1] wrote %zu trace rows to %s\n", G_TRACE.rows.size(), G_TRACE.path.c_str());
    }
