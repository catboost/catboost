...
protected:
    void operator delete(void *) { Y_FAIL(); }
    void operator delete[](void *) { Y_FAIL(); }

    void operator delete(void *, const std::nothrow_t &) { Y_FAIL(); }
    void operator delete[](void *, const std::nothrow_t &) { Y_FAIL(); }
};
...
struct TNalfAllocator
{
    ...
    template<typename U> explicit TNalfAllocator(TNalfAllocator<U, Hint>) noexcept {};
    template<typename U> struct rebind { typedef TNalfAllocator<U, Hint> other; };
...
template <typename Type> using TNalfChunkedAllocator = TNalfAllocator<Type, NNumaAwareLockFreeAllocator::TAllocHint::Chunked>;
template <typename Type> using TNalfIncrementalAllocator = TNalfAllocator<Type, NNumaAwareLockFreeAllocator::TAllocHint::Incremental>;
