...
namespace {
    class TThreadedResolver: public IThreadPool::IThreadAble, public TNonCopyable {
        struct TResolveRequest {
            ...
            inline TNetworkAddressPtr Wait() {
                ...
                if (!Error) {
                    if (!Result) {
                        ythrow TNetworkResolutionError(EAI_AGAIN) << STRINGBUF(": resolver down");
                    }
                ...
                ythrow TNetworkResolutionError(EAI_FAIL) << STRINGBUF(": shit happen");
            }
