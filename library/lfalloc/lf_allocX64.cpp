...
struct TLocalPerTagAllocCounter {
    ...
    {
        AtomicAdd(parent.Size, Size); Size = 0;
        AtomicAdd(parent.Count, Count); Count = 0;
        Updates = 0;
    }
};
...
struct TChunkStats
{
    ...
    TChunkStats(size_t chunk, int64_t size, char *entries)
        : Size(size)
        , Entries(entries)
        , FreeCount(0)
    {
    ...
    void CheckBlock(char *pBlock)
    {
        if (pBlock && pBlock >= Start && pBlock < Finish) {
            ++FreeCount;
            int64_t nShift = pBlock - Start;
            int64_t nOffsetInStep = nShift & (N_CHUNK_SIZE - 1);
            Entries[nOffsetInStep / Size] = 1;
        }
    }
    void SetGlobalFree(char *ptr)
    {
        int64_t nShift = ptr - Start;
        int64_t nOffsetInStep = nShift & (N_CHUNK_SIZE - 1);
...
static void DumpMemoryBlockUtilizationLocked()
{
    ...
    int64_t nTotalAllocated = 0, nTotalFree = 0, nTotalBadPages = 0, nTotalPages = 0, nTotalUsed = 0, nTotalLocked = 0;
    int64_t nTotalGroupBlocks = 0;
    char *entries;
    ...
    for (size_t k = 0; k < N_CHUNKS; ++k) {
        ...
        int64_t nSizeIdx = chunkSizeIdx[k];
        int64_t nSize = nSizeIdxToSize[nSizeIdx];
        ...
        for (int i = 0, nShift = 0; i < nEntriesTotal; ++i, nShift += nSize) {
            ...
            if (entries[i])
                ...
                nBit = 2; // used entry
            for (int64_t nDelta = nSize - 1; nDelta >= 0; nDelta -= N_PAGE_SIZE)
