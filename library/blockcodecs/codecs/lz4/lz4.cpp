    ...
    struct TLz4Registrar {
        TLz4Registrar() {
            for (int i = 0; i < 30; ++i) {
                ...
                if (t1->Methods) {
                    RegisterCodec(t1.Release());
                }

                if (t2->Methods) {
                    RegisterCodec(t2.Release());
                }
            }

            RegisterCodec(new TLz4Codec<TLz4BestCompress, TLz4FastDecompress>());
            RegisterCodec(new TLz4Codec<TLz4BestCompress, TLz4SafeDecompress>());
