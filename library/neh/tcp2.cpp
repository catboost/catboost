...
#ifdef DEBUG_TCP2
    TSpinLock OUT_LOCK;
    #define DBGOUT(args) { TGuard<TSpinLock> m(OUT_LOCK); Cout << TInstant::Now().GetValue() << " " << args << Endl; }
#else
    #define DBGOUT(args)
#endif
...
#define TCP2_TRY_SET(optType, optName) if (name == STRINGBUF(#optName)) {\
        ...
        else TCP2_TRY_SET(size_t, InputBufferSize)
        else TCP2_TRY_SET(size_t, AsioClientThreads)
        else TCP2_TRY_SET(size_t, AsioServerThreads)
        else TCP2_TRY_SET(int, Backlog)
        else TCP2_TRY_SET(bool, ClientUseDirectWrite)
        else TCP2_TRY_SET(bool, ServerUseDirectWrite)
        else TCP2_TRY_SET(TDuration, ServerInputDeadline)
        else TCP2_TRY_SET(TDuration, ServerOutputDeadline)
        else {
            return false;
        }
        return true;
...
#pragma pack(pop)
    ...
    class TServer: public IRequester  {
        ...
        private:
            ...
            void OnError() {
                DBGOUT("Srv OnError(" << (!AS_ ? -666 : AS_->Native()) << ")" << " c=" << (size_t)this);
                Canceled_ = true;
                AS_->AsyncCancel();
            }
        ...
        public:
            ...
            bool FlushOutputBuffers(bool asioThread) {
                ...
                DBGOUT("AsyncWriteResponse: " << " [" << OutputBuffers_.GetIOvec()->Bytes() << "]" << " c=" << (size_t)this);
