...
#define NETLIBA_TRY_SET(optType, optName) if (name == STRINGBUF(#optName)) {\
        ...
        else NETLIBA_TRY_SET(TDuration, AckTailEffect)
        else {
            return false;
        }
        return true;
    ...
    public:
        ...
        void OnSend() {
            if (TNetLibaOptions::AckTailEffect.GetValue()
                    && TLastAckTimes::Common().Get(IdAddr_) + TNetLibaOptions::AckTailEffect.GetValue() > TInstant::Now().GetValue())
            {
