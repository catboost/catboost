...
{
    YT_ASSERT((reinterpret_cast<TPackedPtr::TUnderlying>(Ptr) & PackedPtrTagMask) == 0);
    return TPackedPtr(
        (static_cast<TPackedPtr::TUnderlying>(Tag) << PackedPtrAddressBits) |
        reinterpret_cast<TPackedPtr::TUnderlying>(Ptr));
}
...
{
    return {
        reinterpret_cast<T*>(packedPtr.Underlying() & PackedPtrAddressMask),
        static_cast<ui16>(packedPtr.Underlying() >> PackedPtrAddressBits),
    };
}
