...
#define Y_CPU_BENCHMARK(name, cnt)                                                            \
    namespace N_bench_##name {                                                                \
        static void Run(::NBench::NCpu::TParams&); /* NOLINT(misc-use-anonymous-namespace) */ \
        const ::NBench::NCpu::TRegistar benchmark(#name, &Run);                               \
    }                                                                                         \
    static void N_bench_##name::Run(::NBench::NCpu::TParams& cnt) /* NOLINT(misc-use-anonymous-namespace) */
