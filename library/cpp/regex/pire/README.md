...
Бывают случаи, когда время компиляции регулярки непредсказуемо и может привести к длительному времени компиляции (даже если итоговое число состояний в DFA небольшое) или OOM. Примеры небольших регулярок, которые компилируются долго, потребляют много памяти и приводят к взрывному росту числа состояний (`.*a.{1000}`, `[^a]{2000}`, `.{2500}`, `.*a....................`, `~(.*a.{20})`). И не всегда `Fsm::Determine` помогает, так как "взрыв" происходит до его вызова. Для защиты можно использовать механизм бюджетирования:

```cpp
try {
    Pire::ScopedOperationBudget budget(10'000'000);
    auto first = Pire::Lexer("(abc|def){10}").Parse().Compile<Pire::Scanner>();
    auto second = Pire::Lexer("[a-z]+").Parse().Compile<Pire::Scanner>();
    // Обе компиляции расходуют один бюджет; budget.Remaining() — остаток.
} catch (const Pire::BudgetExceeded&) {
    // Не хватило бюджета для следующей порции работы.
}
// Предыдущий бюджет восстановлен, в том числе при исключении.
```

Число в примере иллюстративное: production-порог выбирает вызывающий код.
Этот бюджет задаёт thread_local-переменную и операции вычитают значения из неё.

Эти абстрактные единицы отражают работу алгоритмов, а не инструкции CPU, байты или время. Их стоимость может меняться вместе с реализацией. Можно считать, что одна единица - это `O(1)` операций. Это защита от дорогих преобразований, а не точный лимит памяти или времени. Поэтому **используйте только с целью защиты от взрывного роста, а не как приблизительную оценку сложности. Устанавливайте достаточно большой бюджет, чтобы избежать ложноположительных срабатываний.**

Чтобы дать хоть какой-то ориентир: `10 000 000` бюджета - это примерно 1 секунда. Время и память сильно варьируются в зависимости от ввода, поэтому для выбора бюджета основывайтесь на своём собственном профиле нагрузки.
