#include "json_value_ordered.h"
#include "json_ordered.h"
...
static bool
AreJsonMapsEqual(const NJson::NOrderedJson::TJsonValue& lhs, const NJson::NOrderedJson::TJsonValue& rhs) {
...
static bool
AreJsonArraysEqual(const NJson::NOrderedJson::TJsonValue& lhs, const NJson::NOrderedJson::TJsonValue& rhs) {
    using namespace NJson::NOrderedJson;
...
namespace NJson::NOrderedJson {
    ...
    TJsonValue::TJsonValue(const NJson::TJsonValue& vval) {
        switch (vval.GetType()) {
            case NJson::EJsonValueType::JSON_ARRAY:
                SetType(JSON_ARRAY);
                for (const auto& item : vval.GetArray()) {
                    AppendValue(TJsonValue(item));
                }
                break;
            case NJson::EJsonValueType::JSON_MAP:
                SetType(JSON_MAP);
                for (const auto& item : vval.GetMap()) {
                    InsertValue(item.first, TJsonValue(item.second));
                }
                break;
            case NJson::EJsonValueType::JSON_BOOLEAN:
                SetType(JSON_BOOLEAN);
                Value.Boolean = vval.GetBoolean();
                break;
            case NJson::EJsonValueType::JSON_INTEGER:
                SetType(JSON_INTEGER);
                Value.Integer = vval.GetInteger();
                break;
            case NJson::EJsonValueType::JSON_UINTEGER:
                SetType(JSON_UINTEGER);
                Value.UInteger = vval.GetUInteger();
                break;
            case NJson::EJsonValueType::JSON_STRING:
                SetType(JSON_STRING);
                Value.String = vval.GetString();
                break;
    ...
    NJson::TJsonValue TJsonValue::GetNonOrderedJsonValue() const {
        NJson::TJsonValue res;
        switch (Type) {
            case JSON_ARRAY:
                res.SetType(NJson::EJsonValueType::JSON_ARRAY);
                for (const auto& item : *Value.Array) {
                    res.AppendValue(item.GetNonOrderedJsonValue());
                }
                return res;
            case JSON_MAP:
                res.SetType(NJson::EJsonValueType::JSON_MAP);
                for (const auto& item : *Value.Map) {
                    res.InsertValue(item.first, item.second.GetNonOrderedJsonValue());
                }
                return res;
            case JSON_BOOLEAN:
                return NJson::TJsonValue(Value.Boolean);
            case JSON_INTEGER:
                return NJson::TJsonValue(Value.Integer);
            case JSON_UINTEGER:
                return NJson::TJsonValue(Value.UInteger);
            case JSON_STRING:
                return NJson::TJsonValue(Value.String);
            case JSON_NULL:
                return NJson::TJsonValue(NJson::EJsonValueType::JSON_NULL);
            case JSON_UNDEFINED:
                return NJson::TJsonValue();
            case JSON_DOUBLE:
                return NJson::TJsonValue(Value.Double);
        }
    }
    ...
    void TJsonValue::Save(IOutputStream* s) const {
        GetNonOrderedJsonValue().Save(s);
    }

    void TJsonValue::Load(IInputStream* s) {
        auto temp = GetNonOrderedJsonValue();
        temp.Load(s);
        *this = TJsonValue(std::move(temp));
    }
...
void Out<NJson::NOrderedJson::TJsonValue>(IOutputStream& out, const NJson::NOrderedJson::TJsonValue& v) {
    NJsonOrderedWriter::TBuf buf(NJsonOrderedWriter::HEM_DONT_ESCAPE_HTML, &out);
