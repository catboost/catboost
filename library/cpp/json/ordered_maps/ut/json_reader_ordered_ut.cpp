#include <library/cpp/json/ordered_maps/json_reader_ordered.h>
#include <library/cpp/json/ordered_maps/json_writer_ordered.h>
...
class TReformatCallbacks: public NJson::TJsonCallbacks {
    TJsonWriter& Writer;
...
Y_UNIT_TEST_SUITE(TJsonReaderTest) {
    ...
    Y_UNIT_TEST(TJsonInvalidTest) {
...

        {
            ...
            in << "{ \"test\" : }";
            TJsonValue value;
            UNIT_ASSERT_EXCEPTION(ReadJsonTree(&in, &value, true), NJson::TJsonException);
        }
    }
    ...
    Y_UNIT_TEST(TJsonMemoryLeakTest) {
        ...
        try {
            TStringInput in(s);
            NJson::NOrderedJson::ReadJsonTree(&in, &json, true);
        } catch (...) {
        }
    } // TJsonMemoryLeakTest
...
Y_UNIT_TEST_SUITE(TCompareReadJsonFast) {
    Y_UNIT_TEST(NoEndl) {
        ...
        bool success = NJson::NOrderedJson::ReadJsonTree(YANDEX_STREAMING_JSON, &parsed, false);
        bool fast_success = NJson::NOrderedJson::ReadJsonFastTree(YANDEX_STREAMING_JSON, &parsed, false);
        UNIT_ASSERT(success == fast_success);
    }
    Y_UNIT_TEST(WithEndl) {
        NJson::NOrderedJson::TJsonValue parsed1;
        NJson::NOrderedJson::TJsonValue parsed2;

        bool success = NJson::NOrderedJson::ReadJsonTree(YANDEX_STREAMING_JSON + "\n", &parsed1, false);
        bool fast_success = NJson::NOrderedJson::ReadJsonFastTree(YANDEX_STREAMING_JSON + "\n", &parsed2, false);
    ...
    Y_UNIT_TEST(NoQuotes) {
        ...
        bool success = NJson::NOrderedJson::ReadJsonTree(streamingJson, &parsed, false);
