...
TString NodeToYsonString(const TNode& node, ::NYson::EYsonFormat format = YF_TEXT);

// Same as the latter, but maps' keys are sorted lexicographically (to be used in tests)
TString NodeToCanonicalYsonString(const TNode& node, ::NYson::EYsonFormat format = YF_TEXT);
...
void NodeToYsonStream(const TNode& node, IOutputStream* output, ::NYson::EYsonFormat format = YF_TEXT);

// Same as the latter, but maps' keys are sorted lexicographically (to be used in tests)
void NodeToCanonicalYsonStream(const TNode& node, IOutputStream* output, ::NYson::EYsonFormat format = YF_TEXT);
