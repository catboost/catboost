...
private:
    ...
    template<class T1, class TT1, ui32 PageSize1, class A1> friend struct TPagedVectorIterator;
public:
    ...
    TPagedVectorIterator(const TPagedVectorIterator<T1, TT1, PageSize1, A1>& it) :
        Offset(it.Offset), Vector(it.Vector) {
    }
...
class TPagedVector: private yvector<TSimpleSharedPtr<yvector<T, A> > , A> {
    static_assert(PageSize, "expect PageSize");

    typedef yvector<T, A> TPage;
    typedef yvector<TSimpleSharedPtr<TPage> , A> TPages;
...
typedef std::is_same<std::random_access_iterator_tag, std::iterator_traits<
        TPagedVector<ui32>::iterator>::iterator_category> TIteratorCheck;
