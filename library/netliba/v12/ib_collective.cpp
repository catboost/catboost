...
struct TMergeRecord
{
    struct TTransfer
    {
        ...
        TTransfer() : DstRank(-1), SL(0), RangeBeg(0), RangeFin(0), Id(0) {}
        TTransfer(int dstRank, int sl, int rangeBeg, int rangeFin, int id) : DstRank(dstRank), SL(sl), RangeBeg(rangeBeg), RangeFin(rangeFin), Id(id) {}
    };
    struct TInTransfer
    {
        int SrcRank;
        ...
        TInTransfer() : SrcRank(-1), SL(0) {}
        TInTransfer(int srcRank, int sl) : SrcRank(srcRank), SL(sl) {}
    };
...
struct TMergePlan
{
    ...
    TMergePlan() : ColSize(0), MaxRankReceiveCount(0) {}
    void Init(int colSize)
    {
        Iterations.resize(0);
...
struct TSRTransfer
{
    ...
    TSRTransfer() { Zero(*this); }
    TSRTransfer(int srcRank, int dstRank, int rangeBeg, int rangeFin) : SrcRank(srcRank), DstRank(dstRank), RangeBeg(rangeBeg), RangeFin(rangeFin) {}
};
...
struct TCoverInterval
{
    ...
    TCoverInterval() : Beg(0), Fin(0) {}
    TCoverInterval(int b, int f) : Beg(b), Fin(f) {}
};
...
public:
    ...
    TAllDataSync& GetDataSync() { return DataSync; }
    void* GetRawData() override { return DataSync.GetRawData(); }
    size_t GetRawDataSize() override { return DataSync.GetRawDataSize(); }
    void Sync() override { DataSync.Sync(); }
    void Flush() override { DataSync.Flush(); }
...
struct TIBAddr
{
    ...
    TIBAddr() : LID(0), SL(0) {}
    TIBAddr(int lid, int sl) : LID(lid), SL(sl) {}
};
...
class TIBCollective : public IIBCollective
{
    struct TPendingMessage
    {
        ...
        TPendingMessage() { Zero(*this); }
        TPendingMessage(int qpn, ui64 wid) : QPN(qpn), WorkId(wid) {}
    };
    ...
    int GetRank() override { return ColRank; }
    int GetSize() override { return ColSize; }
    int GetGroupTypeCount() override { return HostGroup.ysize(); }
