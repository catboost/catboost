...
struct TOutRequestState
{
    ...
    TOutRequestState() : State(S_SENDING), TransferId(0), TimePassed(0) {}
};
...
class TUdpHttp : public IRequester
{
    ...
    struct TTransferPurpose
    {
        EDir Dir;
        TGUID Guid;
        TTransferPurpose() : Dir(DIR_OUT) {}
        TTransferPurpose(EDir dir, TGUID guid) : Dir(dir), Guid(guid) {}
    };
    ...
    struct TStatsRequest : public TThrRefBase
    {
        ...
        TStatsRequest(EReq req) : Req(req), RequestFound(false) {}
    };
...
private:
    void FinishOutstandingTransactions()
    {
        ...
        while ((!OutRequests.empty() || !InRequests.empty() || !SendRespList.IsEmpty() || !SendReqList.IsEmpty())
               && !AtomicAdd(PanicAttack, 0))
        {
...
void AbortOnFailedRequest(TUdpHttpResponse *answer)
{
    if (answer && answer->Ok == TUdpHttpResponse::FAILED) {
        printf("Failed request to host %s\n", GetAddressAsString(answer->PeerAddress).c_str()); fflush(nullptr);
        Y_ASSERT(0);
        abort();
    }
}
