...
#pragma pack(1)
    ...
    struct TCmdDataTiny
    {
        ...
        static int GetMaxDataSize() { return sizeof(TDataVec); }
    };
    struct TCmdDataInit
    {
        char Command;
...
#pragma pack()
    ...
    struct TCompleteInfo
    {
        ...
        TCompleteInfo(int t, int bufId, TIBMsgHandle msg) : Type(t), BufId(bufId), MsgHandle(msg) {}
    };
    ...
    struct TQueuedSend
    {
        ...
        TQueuedSend(const TGUID &packetGuid, TIBMsgHandle msgHandle)
            : PacketGuid(packetGuid), MsgHandle(msgHandle), RemoteAddr(0), RemoteKey(0)
        {
        }
    };
    struct TQueuedRecv
    {
        ...
        TQueuedRecv(const TGUID &packetGuid, const TGUID &connectionGuid, TPtrArg<TIBMemBlock> data)
            : PacketGuid(packetGuid), ConnectionGuid(connectionGuid), Data(data)
        {
        }
    };
