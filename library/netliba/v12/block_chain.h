...
public:
    struct TBlock
    {
        ...
        TBlock() : Data(nullptr), Offset(0), Size(0) {}
        TBlock(const char *data, int offset, int sz) : Data(data), Offset(offset), Size(sz) {}
    };
private:
...
public:
    ...
    const TBlock &GetBlock(int i) const { return Blocks[i]; }
    int GetBlockCount() const { return Blocks.ysize(); }
...
class TBlockChainIterator
{
    ...
    bool Failed;
public:
    TBlockChainIterator(const TBlockChain &chain) : Chain(chain), Pos(0), BlockPos(0), BlockId(0), Failed(false) {}
    ...
    int GetSize() const { return Chain.GetSize(); }
...
class TRopeDataPacket : public TNonCopyable, public TWithCustomAllocator
{
    ...
    static char *AllocBuf(int sz) { return TCustomAllocator<char>().allocate(sz); }
    static void FreeBuf(char *buf) { return TCustomAllocator<char>().deallocate(buf, 0U); }
public:
    TRopeDataPacket() : Block(DefaultBuf), BlockEnd(DefaultBuf + Y_ARRAY_SIZE(DefaultBuf)) {}
    ~TRopeDataPacket()
    {
