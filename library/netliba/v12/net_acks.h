...
{
const float MIN_PACKET_RTT_SKO = 0.002f; // avoid drops due to small hiccups
...
public:
    TPingTracker();
    float GetRTT() const { return AvrgRTT; }
    ...
    float GetTimeout() const { return GetRTT() + GetRTTSKO() * 3; }
...
class TLameMTUDiscovery : public TThrRefBase
{
    ...
    EState State;
public:
    TLameMTUDiscovery() : TimePassed(0), TimeSinceLastPing(0), State(NEED_PING) {}
    bool CanSend() { return State == NEED_PING; }
    void PingSent() { State = WAIT; TimeSinceLastPing = 0; }
    bool IsTimedOut() const { return TimePassed > LAME_MTU_TIMEOUT; }
...
class TCongestionControl : public TThrRefBase
{
    ...
    // ex CONG_CTRL_CHANNEL_INFLATE constant.
    float ChannelInflate;
...
public:
    TCongestionControl(const float channelInflate)
        : ChannelInflate(channelInflate)
        , Window(StartWindowSize * ChannelInflate), PacketsInFly(0), FailRate(0)
        , MinRTT(10), MaxWindow(10000)
        , FullSpeed(false)
        , DoCountTime(false)
        , TimeSinceLastRecv(0)
        , AvrgRTT(0)
        , HighRTTCounter(0)
        , WindowFraction(0), FractionRecalc(0)
    ...
    void LaunchPacket() { PacketsInFly += 1.0f; TimeWindow -= 1.0f; }
    void RegisterRTT(float RTT)
    {
        if (RTT < 0)
            return;
        ...
        if (RTT > AvrgRTT) {
            ...
            if (HighRTTCounter >= CONG_CTRL_RTT_SEQ_COUNT) {
                ...
                if (FullSpeed) {
                    float windowSubtract = Window * ((1 - CONG_CTRL_WINDOW_SHRINK_RTT) / ChannelInflate);
    ...
    void Success()
    {
        ...
        if (FullSpeed && VirtualPackets == 0) {
            ...
            Window += sqrt(Window / ChannelInflate) * CONG_CTRL_WINDOW_GROW;
            if (UseTOSforAcks) {
    ...
    void FailureOnSend()
    {
        ...
        // It is a bit dirty solution of problem with simultaneous transfer
        // via slow and fast network link:
        // The issue is we put a lot of packets for "slow destination"
        // in to socket queue (in kernel), and this queue will be dequeuing
        // with speed of "slow network card". Eventually socket queue will be
        // overflowed.
        // Calling Failure() here increases window for "slow destinations"
        // and prevents overflow
        Failure();
    }
    void Failure(bool updateFailRate = true)
    {
        ...
        if (TimeSinceLastFail > CONG_CTRL_MIN_FAIL_INTERVAL) {
            ...
            if (Window <= CONG_CTRL_MIN_WINDOW) {
                ...
                float windowSubtract = Window * ((1 - CONG_CTRL_WINDOW_SHRINK) / ChannelInflate);
                Window = Max(CONG_CTRL_MIN_WINDOW, Window  - windowSubtract);
                VirtualPackets = Max(0.f, VirtualPackets - windowSubtract);
            }
        }
    ...
    bool HasPacketsInFly() const { return PacketsInFly > 0; }
    float GetTimeout() const { return PingTracker.GetTimeout(); }
    float GetWindow() const { return Window; }
    float GetRTT() const { return PingTracker.GetRTT(); }
    float GetFailRate() const { return FailRate; }
    float GetTimeSinceLastRecv() const { return TimeSinceLastRecv; }
    float GetMaxWindow() const { return UseTOSforAcks ? MaxWindow : -1; }
    void MarkAlive()
    {
        TimeSinceLastRecv = 0;
        if (PortTester.Get())
    ...
    bool IsAlive() const { return TimeSinceLastRecv < 1e6f; }
    void Kill() { TimeSinceLastRecv = 1e6f; }
    ...
    bool IsKnownMTU() const { return MTU != 0; }
...
public:
    ...
    typedef THashMap<int, float, THash<int>, TEqualTo<int>, TCustomAllocator<std::pair<const int, float>>> TPacketHash;
    TPacketHash PacketsInFly, DroppedPackets;
    TVector<int, TCustomAllocator<int>> ResendQueue;
    TIntrusivePtr<TCongestionControl> Congestion;
    TVector<bool, TCustomAllocator<bool>> AckReceived;
    bool IsCanceled;
    float TimeToNextPacketTimeout;

    int SelectPacket();
    TAckTracker() : PacketCount(0), CurrentPacket(0), IsCanceled(false), TimeToNextPacketTimeout(1000) {}
    ~TAckTracker();
    void AttachCongestionControl(TCongestionControl *p) { Congestion = p; }
    TIntrusivePtr<TCongestionControl> GetCongestionControl() const { return Congestion.Get(); }
    bool IsInitialized() { return PacketCount != 0; }
    int GetPacketCount() const { return PacketCount; }
    void SetPacketCount(int n) { Y_ASSERT(PacketCount == 0); PacketCount = n; AckReceived.resize(n, false); }
    int GetPacketToSend(float deltaT, bool* isCanceled);
    void Cancel();
    void AddToResend(int pkt); // called when failed to send packet
    void EraseFromResend(int pkt);
    void Resend();
    void Ack(int pkt, float deltaT, bool updateRTT);
    void AckAll();
    void MarkAlive() { Congestion->MarkAlive(); }
    bool IsAlive() const { return Congestion->IsAlive(); }
    void Step(float deltaT);
    bool CanSend() const { return Congestion->CanSend(); }
    float GetTimeToNextPacketTimeout() const { return TimeToNextPacketTimeout; }

    const decltype(AckReceived)& GetAckReceived() const { return AckReceived; }
};
