...
private:
    ...
    typedef TMap<T, T> TTree;  // [key, value)
    typedef typename TTree::iterator TIterator;
    typedef typename TTree::reverse_iterator TReverseIterator;
    typedef TDisjointIntervalTree<T> TThis;
...
private:
    enum EState { S_HOLE = 0, S_ACTIVE = 1, S_OK = 2, S_FAILED = 3, S_CANCELED = 4 };
