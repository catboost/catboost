...
{
#define CHECK_Z(x) { int rv = (x); if (rv != 0) { fprintf(stderr, "check_z failed, errno = %d\n", errno); Y_VERIFY(0, "check_z"); } }
...
public:
    ...
    bool IsValid() const { return Context != nullptr && ProtDomain != nullptr; }
    ...
    class TLock
    {
        ...
        TGuard<TMutex> Guard;
    public:
        TLock(TPtrArg<TIBContext> ctx) : Ptr(ctx), Guard(ctx->Lock) {}
        ibv_context *GetContext() { return Ptr->Context; }
        ibv_pd *GetProtDomain() { return Ptr->ProtDomain; }
    };
};
...
public:
    ...
    TIBContext *GetCtx() { return IBCtx.Get(); }
...
public:
        ...
        : IBCtx(ctx)
    {
        ...
        mrInfo.length = len;
        mrInfo.exp_access = IBV_EXP_ACCESS_LOCAL_WRITE
    ...
    ui32 GetLKey() const { static_assert(sizeof(ui32) == sizeof(MR->lkey), "expect sizeof(ui32) == sizeof(MR->lkey)"); return MR->lkey; }
    ui32 GetRKey() const { static_assert(sizeof(ui32) == sizeof(MR->lkey), "expect sizeof(ui32) == sizeof(MR->lkey)"); return MR->lkey; }
    char *GetData() { return MR ? (char*)MR->addr : nullptr; }
...
public:
    ...
    TAddressHandle(TPtrArg<TIBPort> port, sockaddr &remoteAddr, sockaddr &localAddr, int serviceLevel)
        : IBCtx(port->GetCtx())
    {
        ibv_ah_attr attr;
        if (MakeAH(&attr, port, remoteAddr, localAddr, serviceLevel)) {
            TIBContext::TLock ibContext(IBCtx);
            AH = ibv_create_ah(ibContext.GetProtDomain(), &attr);
            Y_ASSERT(AH != nullptr);
        } else {
    ...
    bool IsValid() const { return AH != nullptr; }
};
...
public:
    int GetQPN() const { if (QP) return QP->qp_num; return 0; }
    int GetPSN() const { return MyPSN; }
    // we are using srq
...
public:
    ...
    int GetLID() const { return 1; }
    TIBContext *GetCtx() { return 0; }
    void GetGID(ibv_gid *res) { Zero(*res); }
...
public:
    ...
    ui32 GetLKey() const { return 0; }
    ui32 GetRKey() const { return 0; }
    char *GetData() { return 0; }
    bool IsCovered(const void *, size_t ) const { return false; }
};
...
inline TIntrusivePtr<TIBPort> GetIBDevice() { return 0; }
#endif
}
