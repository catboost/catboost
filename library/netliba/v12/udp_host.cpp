...
typedef std::function<void (const TTransfer& transfer,
    TAutoPtr<TRopeDataPacket> data, EPacketPriority pp, const TTos& tos, ui8 netlibaColor)> TSendCb;
...
class TTXUserQueue {
    ...
    struct TSendCmd : public TWithCustomAllocator {
        TSendCmd(const TTransfer& transfer, TAutoPtr<TRopeDataPacket> data, EPacketPriority pp, const TTos& tos, ui8 netlibaColor)
            :Transfer(transfer), Data(std::move(data)), Pp(pp), Tos(tos), NetlibaColor(netlibaColor) {}
    ...
    struct TCancelCmd : public TWithCustomAllocator {
        explicit TCancelCmd(const TTransfer& transfer) : Transfer(transfer) {}
        const TTransfer Transfer;
    };
        ...
        for (TDeque<ui64>::iterator z = transferIdQueue.begin(); z!= transferIdQueue.end();) {
            ...
            if (!xfer) {
                ...
                if (transferIdQueue.empty()) {
                    return connection->HasAnySendingTransfers() ?
                                TSentConnectionResult::SCR_CONT : TSentConnectionResult::SCR_DELETE; //delete connection if all queues are empty
                } else {
                    continue;
                }
            } else {
                                         ...
                                         const sockaddr_in6& fromAddress, const sockaddr_in6& dstAddress, TAutoPtr<TUdpRecvPacket> recvBuf)
{
    ...
    return IsTransferCmd(cmd) ?
               ProcessTransferPacket(cmd, pktData, pktEnd, recvBuf, connection, thatSideHasChanged, opt) :
               ProcessPingPacket(cmd, pktData, pktEnd, fromAddress, connection);
}
                                     ...
                                     TAutoPtr<TUdpRecvPacket> recvBuf, TConnection* connection, const bool thatSideHasChanged, const TOptionsVector& opt)
{
    ...
    ui64 transferId;
    if (!ReadTransferHeader(&pktData, pktEnd, &transferId, opt) ||
        !transferId)  // validation
    {
        Y_ASSERT(false);
        return false;
    }
                                 ...
                                 TAutoPtr<TUdpRecvPacket> recvBuf, const TTransfer& transfer, const TOptionsVector& opt)
{
    ...
    if (!ReadPacketHeaderTail(&pktData, pktEnd, &packetId, &ackTos, &netlibaColor, opt) ||
        packetId < 0 || !IsValidTos(ackTos))
    {
        Y_ASSERT(false);
        return false;
    }
