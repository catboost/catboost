...
public:
    ...
    TUdpInTransfer() : PacketSize(0), LastPacketSize(0), HasLastPacket(false), TimeSinceLastRecv(0), Attempt(0), Crc32(0), Stats(nullptr)
    {
        Zero(ToAddress);
    }
    ~TUdpInTransfer()
...
struct TUdpOutTransfer
{
    ...
    TUdpOutTransfer() : PacketCount(0), PacketSize(0), LastPacketSize(0), Attempt(0), Crc32(0), PacketPriority(PP_LOW), Stats(nullptr)
    {
        Zero(ToAddress);
    }
    ~TUdpOutTransfer()
...
public:
    TUdpHostRevBufAlloc() { AllocNewBuf(); }
    ~TUdpHostRevBufAlloc() { FreeBuf(RecvPktBuf); }
    ...
    int GetBufSize() const { return Y_ARRAY_SIZE(RecvPktBuf->Data); }
    char *GetDataPtr() const { return RecvPktBuf->Data; }
};
...
class TUdpHost : public IUdpHost
{
    struct TPeerLink
    {
        ...
        TPeerLink() : TimeNoActiveTransfers(0) {}
