...
public:
    TCongestionControl()
        : Window(StartWindowSize * CONG_CTRL_CHANNEL_INFLATE), PacketsInFly(0), FailRate(0)
        , MinRTT(10), MaxWindow(10000)
        , FullSpeed(false)
        , DoCountTime(false)
        , TimeSinceLastRecv(0)
        , ActiveTransferCount(0), AvrgRTT(0)
        , HighRTTCounter(0)
    ...
    int GetTransferCount() const { return ActiveTransferCount; }
...
public:
    ...
    ~TCongestionControlPtr() { Dec(); }
    TCongestionControlPtr(TCongestionControl *p) : Ptr(p) { Inc(); }
    TCongestionControlPtr &operator=(const TCongestionControlPtr &a) { Dec(); Ptr = a.Ptr; Inc(); return *this; }
    TCongestionControlPtr &operator=(TCongestionControl *a) { Dec(); Ptr = a; Inc(); return *this; }
    operator TCongestionControl*() const { return Ptr.Get(); }
    TCongestionControl* operator->() const { return Ptr.Get(); }
    TIntrusivePtr<TCongestionControl> Get() const { return Ptr; }
};
...
class TAckTracker
{
    struct TFlyingPacket
    {
        float T;
        int PktId;
        TFlyingPacket() : T(0), PktId(-1) {}
        TFlyingPacket(float t, int pktId) : T(t), PktId(pktId) {}
    };
    ...
    int SelectPacket();
public:
    TAckTracker() : PacketCount(0), CurrentPacket(0), TimeToNextPacketTimeout(1000) {}
    ~TAckTracker();
