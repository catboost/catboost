...
struct TIBMemSuperBlock : public TThrRefBase, TNonCopyable
{
    ...
    char *GetData();
    size_t GetSize() { return ((ui64)1) << SzLog; }
    void IncRef()
    {
...
public:
    TIBMemBlock(TPtrArg<TIBMemSuperBlock> super, char *data, size_t sz)
        : Super(super), Data(data), Size(sz)
    {
        Super->IncRef();
    }
    TIBMemBlock(size_t sz)
        : Super(nullptr), Size(sz)
    {
        // not really IB mem block, but useful IB code debug without IB
        Data = new char[sz];
    }
    char* GetData() { return Data; }
    ui64 GetAddr() { return Data - (char*)nullptr; }
    size_t GetSize() { return Size; }
    TMemoryRegion* GetMemRegion() { return Super.Get() ? Super->MemRegion.Get() : nullptr; }
};
...
private:
    ...
    public:
        ...
        TIBMemSuperBlock *Get() { return Blk.Get(); }
    };
...
public:
    ...
    TIBContext* GetIBContext() { return IBCtx.Get(); }
