...
- **#128** [pending] **S35-Q4-L2-PARENT-TERM**: investigate L2 parent-term divergence MLX vs CPU. MLX's L2 path subtracts `totalSum²/(totalWeight+λ)` per-(p, k) at `csv_train.cpp:1704` (one-hot) and `:1973` (ordinal); CPU's L2 path (`score_calcers.cpp:20-33`) subtracts no parent term. Argmax-invariant at depth=0 numPartitions=1 (constant offset) but variable at depth ≥ 1 or numPartitions > 1. Currently papered over by 18-config L2 parity gate G4d (which measures aggregate RMSE drift, not split-selection agreement). Scoping: split-selection harness (S31-T3b style) at depth ≥ 1 to surface or refute. Source: `docs/sprint34/probe-f-lite/math-derivation.md` §8.2 Q4. blockedBy: none.
