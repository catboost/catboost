...
- **#129** [pending] **S35-1H-L2-PER-SIDE**: apply per-side mask to one-hot L2 branch at `csv_train.cpp:1704+` (mirror of Commit 1.5 but L2-only — leave Cosine joint-skip per S34 verdict). DEC-042-class fix: parent term cancels at degenerate (p,k) → math no-op-but-correct, matches CPU. The S33-S1 in-session attempt failed because it applied per-side mask to BOTH L2 and Cosine; this ticket applies it to L2 only. Ordinal already shipped in Commit 1.5; one-hot is the symmetric pair. Source: S35-Q4 math derivation §9. blockedBy: none.
