...
> Last updated: 2026-04-24 (Sprint 33 RETRACTED — L4 falsified; PROBE-C STAGE 2 LANDS the new narrowing: divergence is at iter=2 **depth=2**, not depth=0; L3 depth-0 verdict was a coordinate-system error; DEC-036 OPEN, DEC-041 INVALIDATED)
...
- **Successor scope** (next session, not S33): per-feature gain dump at iter=2 depth=2 both sides (with the d=0+d=1 partitions from the agreed splits) — find the first feature where the gain ranking differs. Three candidate loci: (a) Cosine gain formula precision at depth=2 (joint-denominator drift on 4 partitions), (b) partition-state-vector accumulation differing across the 4 leaves at d=2, (c) histogram bin-edge inclusivity (≤ vs <) for the 1 missing border letting docs migrate. DEC-042 (or later) opens when mechanism is identified — DEC-041 is a dead number.
