...
> Last updated: 2026-04-24 (Sprint 33 RETRACTED — L4 falsified; PROBE-C STAGE 2 narrows to iter=2 **depth=2**; **PROBE-D falsifies precision-class hypothesis** — fp32/fp64 argmax bit-matches at every d=0..5, max residual 3.89e-5 vs 20+ gain gap. Mechanism is **partition-state class** (degenerate-split semantics); next: PROBE-E per-leaf cosNum/cosDen accounting. DEC-036 OPEN, DEC-041 INVALIDATED.)
...
- **PROBE-D result (2026-04-24)** (`docs/sprint33/probe-d/FINDING.md`, `data/cos_accum_seed42_depth{0..5}.csv`): fp32-vs-fp64 gain shadow at iter=2 d=0..5, 2540 (feature, bin) cells per depth. Max abs(g_f32−g_f64) = 3.89e-5; argmax(g_f32)==argmax(g_f64) at every depth. **Smoking gun**: at d=2, 18 noise features cluster at gain ~101.95, signal feat=0 evaluates to 81.89, feat=1 to 77.77 — signal/noise gain INVERSION. CPU's d=2 pick (feat=0, bin=21, border=−0.946874) is bin-mapped ULP-identically and scores 81.887 in MLX; MLX's winner (feat=10, bin=79) scores 101.954. Gap = **20.07 gain units** — three orders of magnitude bigger than any plausible fp32 noise. **Precision-class hypothesis closed.** Loci (a) drops; loci (b)/(c) re-rank: the new lead is **partition-state class — degenerate-split (empty-child) semantics at d≥2**. CPU's d=2 split is degenerate in 2 of 4 leaves (after d=0 splits feat=0 at 0.014, those 2 leaves can't have any X[0]<−0.947); noise features split all 4 leaves non-trivially. MLX vs CPU likely differ in how empty-child contributions enter the joint denominator.
- **Successor scope** (next probe, S33-PROBE-E #126): per-leaf (cosNum, cosDen, doc-count, child-L-count, child-R-count) at iter=2 d=2 for two specific candidates — (feat=0, bin=21) [degenerate] vs (feat=10, bin=79) [non-degenerate] — both sides. Goal: locate the line where MLX includes (or CPU excludes) zero-doc child weight in the joint sums. Likely a one-line fix in score reducer once located. DEC-042 (or later) opens when mechanism identified — DEC-041 stays dead.
