...
void TEnumParser::Parse(const char* data, size_t length) {
    Y_ENSURE(
        TStringBuf(data, length).Contains("#pragma once") ||
        TStringBuf(data, length).Contains("// Generated by the protocol buffer compiler"),
        "Header file containing enums must have #pragma once at beginning, see SEARCH-975. "
    );
