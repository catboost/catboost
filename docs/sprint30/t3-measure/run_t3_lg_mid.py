#!/usr/bin/env python3
"""
S30-T3-MEASURE: Gate G3b — LG-Mid post-Kahan parity (t5-continuity cell).

Cell: N=1000, depth=6, max_leaves=31, iterations=50, seeds={42,43,44,45,46}
...
PASS criterion: all 5 seeds have drift ratio MLX_RMSE / CPU_RMSE in [0.98, 1.02].

This is the "t5-continuity" cell matching the S28 t5 measurement that originally
surfaced the 14% LG Cosine/L2 algorithmic gap (DEC-032). That gap was an
algorithmic mismatch (MLX computing L2, CPU computing Cosine). Post-S28/S30 both
sides compute Cosine. Success here validates the same surface that flagged the bug.

CPU reference: catboost pip, score_function='Cosine', max_leaves=31.
"""
...
def main():
    ...
    print(f"G3b — LG-Mid post-Kahan parity (t5-continuity)")
    ...
    print(f"  PASS criterion: all seeds in [{G3b_PASS_LO}, {G3b_PASS_HI}]\n")
    ...
    for seed in SEEDS:
        ...
        seed_pass = G3b_PASS_LO <= ratio <= G3b_PASS_HI
        ...
        results.append({"seed": seed, "ratio": ratio, "pass": seed_pass})
        raw_rows.append({
            "seed": seed,
            "mlx_rmse": mlx_rmse,
            "cpu_rmse": cpu_rmse,
    ...
    # Write raw CSV
    out_path = DATA_DIR / "g3b_lg_mid.csv"
    ...
    print(f"\n=== G3b RESULT ===")
    for r in results:
        print(f"  seed={r['seed']}: ratio={r['ratio']:.6f}  [{'PASS' if r['pass'] else 'FAIL'}]")
    print(f"  Threshold: [{G3b_PASS_LO}, {G3b_PASS_HI}]")
    print(f"  G3b: {'PASS' if all_pass else 'FAIL'}")

    if not all_pass:
        failed = [r["seed"] for r in results if not r["pass"]]
        print(f"\nG3b FAIL — seeds {failed} outside [{G3b_PASS_LO}, {G3b_PASS_HI}]", file=sys.stderr)
        sys.exit(1)
