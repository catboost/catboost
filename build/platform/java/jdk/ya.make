RESOURCES_LIBRARY()


IF(USE_SYSTEM_JDK)
    MESSAGE(WARNING System JDK $USE_SYSTEM_JDK will be used)
ELSEIF(JDK_VERSION STREQUAL "12")
    IF(HOST_OS_LINUX)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:899341715)
    ELSEIF(HOST_OS_WINDOWS)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:899348970)
    ELSEIF(HOST_OS_DARWIN)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:899346226)
    ELSE()
        MESSAGE(FATAL_ERROR Unsupported platform for JDK12)
    ENDIF()
ELSEIF(JDK_VERSION STREQUAL "11")
    IF(HOST_OS_LINUX)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:810391789)
    ELSEIF(HOST_OS_WINDOWS)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:810454054)
    ELSEIF(HOST_OS_DARWIN)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:810389307)
    ELSE()
        MESSAGE(FATAL_ERROR Unsupported platform for JDK11)
    ENDIF()
    IF(SANITIZER_TYPE STREQUAL "address")
        IF(HOST_OS_LINUX)
            DECLARE_EXTERNAL_RESOURCE(JDK_FOR_TESTS sbr:893693905)
        ELSE()
            MESSAGE(FATAL_ERROR Unsupported platform for JDK11 with asan)
        ENDIF()
    ENDIF()
ELSEIF(JDK_VERSION STREQUAL "11Shenandoah")
    IF(HOST_OS_LINUX)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:898173174)
    ELSE()
        MESSAGE(FATAL_ERROR Unsupported platform for JDK11 with shenandoah gc)
    ENDIF()
ELSEIF(JDK_VERSION STREQUAL "10" OR JDK10) # !JDK10 flag is deprecated, this check should be removed later
    IF(HOST_OS_LINUX)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:545649806)
    ELSEIF(HOST_OS_DARWIN)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:545649998)
    ELSEIF(HOST_OS_WINDOWS)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:545648079)
    ELSE()
        MESSAGE(FATAL_ERROR Unsupported platform for JDK10)
    ENDIF()
ELSEIF(JDK_VERSION STREQUAL "8")
    IF(HOST_OS_LINUX)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:810373574)
    ELSEIF(HOST_OS_DARWIN)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:810369364)
    ELSEIF(HOST_OS_WINDOWS)
        DECLARE_EXTERNAL_RESOURCE(JDK sbr:810375867)
    ELSE()
        MESSAGE(FATAL_ERROR Unsupported platform for JDK8)
    ENDIF()
ELSE()
    MESSAGE(FATAL_ERROR Unsupported JDK version)
ENDIF()

END()
